...
FORBIDDEN_WORKFLOW_FRAGMENTS = (".agents/skills/coding/tests", ".agents/skills/coding/references/")
FORBIDDEN_MANAGED_INTERNALS = ("Runtime Mode", "Source Mode", "研发治理 MCP", "规则标识", "路由映射", "加载明细", "内部凭据")
FORBIDDEN_PROJECT_OVERLAY_GOVERNANCE = ("Runtime Mode", "Source Mode", "研发治理 MCP", "Project Payload", "Runtime Skill Projection", "canonical Reference", ".agents/skills/router/")
PROJECT_DOC_RULES = Path("docs/AGENTS.md")
FORBIDDEN_PROJECT_DOC_GOVERNANCE = (".agents/skills/coding/", "Coding Skill", "Runtime Mode", "Source Mode", "研发治理 MCP")
...
REQUIREMENT_FORM_FIELDS = ("id: objective", "id: scope", "id: non_goals", "id: acceptance_criteria", "id: invariants", "id: upstream_sources", "id: validation_requirements")
BUG_FORM_FIELDS = ("id: actual_behavior", "id: expected_behavior", "id: impact_scope", "id: reproduction_steps", "id: evidence", "id: regression_scope", "id: acceptance_criteria", "id: upstream_sources", "id: validation_requirements")
TECHNICAL_CHANGE_FORM_FIELDS = ("id: motivation", "id: current_state", "id: target_state", "id: scope", "id: non_goals", "id: compatibility_migration", "id: risks_rollback", "id: acceptance_criteria", "id: validation_requirements", "id: upstream_sources")
ISSUE_FORM_PROFILES = {"01-requirement.yml": ("需求", "[需求] "), "02-bug.yml": ("缺陷", "[缺陷] "), "03-technical-change.yml": ("技术变更", "[技术变更] ")}
...
def _check_issue_form(path: Path, required_fields: tuple[str, ...]) -> list[str]:
    ...
    if acceptance is not None and ("label: 验收标准" not in acceptance or "- [ ] AC1：" not in acceptance):
        errors.append(f"GOV017 {path.as_posix()}: acceptance_criteria 必须使用统一验收标准与 AC1 task list")
...
def check_repository(root: Path = ROOT) -> list[str]:
        ...
        if sections is None:
            ...
            for fragment in FORBIDDEN_MANAGED_INTERNALS:
                if fragment in managed:
                    errors.append(f"GOV009 AGENTS.md: managed block 不应展开治理实现细节 {fragment}")
            for fragment in FORBIDDEN_PROJECT_OVERLAY_GOVERNANCE:
                if fragment in project_owned:
                    errors.append(f"GOV011 AGENTS.md: 项目自有 Overlay 不应保存通用治理实现说明 {fragment}")
    ...
    if project_docs.is_file():
        ...
        for fragment in FORBIDDEN_PROJECT_DOC_GOVERNANCE:
            if fragment in docs_text:
                errors.append(f"GOV010 {PROJECT_DOC_RULES.as_posix()}: 项目文档规则不得把本地通用治理安装资产当规则入口 {fragment}")
    ...
    if not (root / PR_REQUIREMENT_SOURCE_CHECK).is_file():
        errors.append(f"GOV015 {PR_REQUIREMENT_SOURCE_CHECK.as_posix()}: PR Requirement Source 机器门禁不存在")
    ...
    for workflow in _workflow_paths(root):
        ...
        for fragment in FORBIDDEN_WORKFLOW_FRAGMENTS:
            if fragment in text:
                errors.append(f"GOV005 {relative}: 永久 CI 不得依赖 Agent_Skills canonical 内部路径 {fragment}")

    completion_owner = root / COMPLETION_OWNER
    if not completion_owner.is_file():
        errors.append(f"GOV006 {COMPLETION_OWNER.as_posix()}: Change Completion / Requirement CI Owner 不存在")
    else:
        ...
        if f"python {READY_CHECK.as_posix()}" in gate_text:
            errors.append(f"GOV016 {COMPLETION_OWNER.as_posix()}: Workflow 不得绕过项目 carrier 直接调用 generic ready-check")
        if "--changed-since" not in gate_text or "--require-active-ready" not in gate_text:
            errors.append(f"GOV016 {COMPLETION_OWNER.as_posix()}: PR changed-since 与 main active-ready 模式必须同时保留")
        ...
        if "check_pr_requirement_source.py" not in gate_text:
            errors.append(f"GOV015 {COMPLETION_OWNER.as_posix()}: 必须继续执行真实 PR Requirement Source 校验")
        if "issues: read" not in gate_text:
            errors.append(f"GOV015 {COMPLETION_OWNER.as_posix()}: PR Requirement Source 校验需要最小 issues: read 权限")
        if "types:" not in gate_text or "- edited" not in gate_text:
            errors.append(f"GOV015 {COMPLETION_OWNER.as_posix()}: PR 正文 edited 后必须重新执行 Requirement Source 校验")
    ...
    errors.extend(_check_issue_form(root / TECHNICAL_CHANGE_ISSUE_FORM, TECHNICAL_CHANGE_FORM_FIELDS))
    ...
    if not pr_template.is_file():
        ...
        if "#123" not in pr_text or "仓库内真实存在" not in pr_text:
            errors.append(f"GOV014 {PR_TEMPLATE.as_posix()}: 必须公开机器可验证的 Issue / 仓库路径来源格式")
        post_merge_markers = ("需要 post-merge evidence", "不得使用 `Closes` / `Fixes` / `Resolves`", "Closure Audit")
        if any(marker not in pr_text for marker in post_merge_markers):
            errors.append(f"GOV014 {PR_TEMPLATE.as_posix()}: post-merge Evidence / Closure Audit 时序缺失")
    return errors
