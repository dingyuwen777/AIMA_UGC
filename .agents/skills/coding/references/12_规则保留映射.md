...
## 14. Figma 设计规则的跨 Skill 归属

Figma 页面、Canvas、Spacing、Annotation、Section、Prototype 和 Design-to-Code 的**设计细则**不属于 Coding 的实现规范。当前 canonical 位置固定为：

```text
.agents/skills/figma/SKILL.md
→ .agents/skills/figma/references/07_页面布局与真实可用性审计.md
→ 其它由 Figma Skill 按任务触发的 references
```

Coding 只在 [02_跨项目研发任务路由.md](02_跨项目研发任务路由.md) 保留跨 Skill 硬路由：同仓存在 Figma Skill 时，Figma 创建、修改、整理、审查、Prototype 或 Design-to-Code 必须进入 Figma Skill；Coding 继续负责仓库事实、Change、实现、验证、Review、CI、Git 和交付，**不得在 Coding references 下恢复第二套 Figma 设计规则**。

Figma Canvas/Spacing/Annotation 规则的内容守恒至少包括：

- 项目 Design System / Spacing Token / Grid / Layout Guideline 优先；通用数值只能是无项目规范时的 fallback；
- 4px 候选网格与页面“同组靠近、异组拉开”的语义间距；
- Annotation / 开发说明与正式 Frame 的边界、归属和 24–32px fallback；
- 连续状态稿、同组画板、跨业务组的 40–64px / 64–80px / 96–160px fallback；
- 正式稿、状态稿、探索稿、说明、历史稿和废弃稿的分区；
- 每次 Figma 写操作后的 Canvas-level Review：当前 Frame、所在 Section、上下左右相邻画板、Annotation 和 zoom-out 整体视图；
- 工具可读几何时优先检查真实 `x / y / width / height / gap / padding / layout`，只有截图时明确精度限制；
- 当前修改直接造成的邻接问题属于同一任务最小修复范围，但不得借机无边界重排整个文件；
- 不得为了画布整洁静默删除历史/备份/废弃稿；
- 页面内部正确但 Canvas 仍拥挤、贴边、遮挡或归属不清时，不得声明 Figma 修改完成。

以后若继续重组 Figma Skill，应在 Figma 自身规则中保持这些语义，并由治理测试从 `Coding → Figma Skill → Figma layout/canvas reference` 验证可达性；不能因为 Coding 不再保存详细正文就把这批规则视为已删除。
