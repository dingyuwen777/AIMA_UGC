...
| --- | --- | --- |
| `project-discovery.md` | `01_项目发现与可失效缓存.md` | 首次进入、缓存缺失/过期、仓库事实发现；职责不变 |
| `change-management.md` | `04_轻量变更管理.md` | L2/L3、需求追踪、Active Change；Validation Matrix 默认语义指向通用策略 |
| `completion-gate.md` | `10_完成定义追溯门禁.md` | Requirement Traceability、Completion Audit、Ready；职责不变 |
| `development-workflows.md` | `05_设计实施与根因调试.md` | Feature/Bug/Refactor/Debug/TDD/最小实现/注释/日志/依赖/Git/文档；职责不变 |
| `repository-constraints.md` | `06_仓库边界数据交换与条件式约束.md` | 只在仓库实际存在时应用 Owner/Contract/Schema/Migration/边界；继续禁止虚构架构 |
| `testing-strategy.md` | `08_分层测试与验收策略.md` | Browser/API/PostgreSQL/Full-stack/Provider 分层专项细节；仅在这些真实边界存在时加载 |
| `collaboration.md` | `09_多人和多智能体并行协作.md` | 多人/多 Agent/多 Change 冲突预检与分工；职责不变 |
| `verification-review.md` | `11_两阶段复核与完成前验证.md` | Requirement review、代码质量、Ready、证据、交付报告；职责不变 |
...
当前：`05_设计实施与根因调试.md` + `07_通用验证与证据策略.md`；Web/API/PostgreSQL/Provider 的原始具体规则继续在 `08_分层测试与验收策略.md`。
