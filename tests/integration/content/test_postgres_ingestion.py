...
def _insert_completed_attempt(
    ...
    *,
    ...
    return run_id, scope_id, request_id, attempt_id


def _mapping_context(
    *, request_id: UUID, attempt_id: UUID, artifact_id: UUID, operation: str, root: str | None = None
) -> XhsMappingContext:
...
def test_real_search_fixture_ingests_content_and_candidate_lineage(
    ...
    try:
        with session.begin():
            _, scope_id, request_id, attempt_id = _insert_completed_attempt(
                session,
                operation="search_notes",
                source_value="爱玛",
                storage_key=f"raw/test/{uuid4()}.json.gz",
            )
        ...
        assert lineage[0] == "note:note-fixture-1"
        assert lineage[1] == attempt_id
        assert lineage[2] == request_id
        assert lineage[3] == scope_id
        assert lineage[5] == artifact_id
    finally:
        session.rollback()
        session.close()
...
def test_postgres_ingestion_preserves_sparse_fields_a_b_a_metrics_and_comment_tree(
    ...
    try:
        with session.begin():
            _, _, request_id, attempt_id = _insert_completed_attempt(
                session,
                operation="search_notes",
                source_value="爱玛",
                storage_key=f"raw/test/{uuid4()}.json.gz",
            ...
            first = service.ingest_content(map_content({**base, "title": "A"}, context, item_locator="a"))
            ...
            sparse_context = XhsMappingContext(
                **{
                    **context.__dict__,
            ...
            _, _, comment_request_id, comment_attempt_id = _insert_completed_attempt(
                session,
                operation="get_note_comments",
                source_value="note-history",
                storage_key=f"raw/test/{uuid4()}.json.gz",
            )
            comment_artifact_id = session.execute(
                select(provider_request_attempts_table.c.raw_artifact_id).where(
                    provider_request_attempts_table.c.id == comment_attempt_id
                )
            ).scalar_one()
            comment = map_comment(
                {
        ...
        current = session.execute(
            select(contents_table).where(contents_table.c.id == first.target_id)
        ).mappings().one()
...
def test_candidate_requires_completed_attempt_with_linked_raw(
    ...
    try:
        with pytest.raises(IntegrityError):
            with session.begin():
                _, _, _, attempt_id = _insert_completed_attempt(
                    session,
                    operation="search_notes",
                    source_value="爱玛",
                    storage_key=f"raw/test/{uuid4()}.json.gz",
                ...
                session.execute(
                    provider_request_attempts_table.update()
                    .where(provider_request_attempts_table.c.id == attempt_id)
                    .values(dispatch_status="dispatching", completed_at=None, raw_artifact_id=None)
                )
