...
def test_scope_runtime_fetches_sub_comments_through_formal_operation(
    ...
    try:
        with session.begin():
            provider_config = PostgresProviderConfigRepository(session).create(
                ProviderConfig(
                    ...
                    secret_ref="providers/tikhub/test/scope-replies-runtime",
                    enabled=True,
                )
            )
        ...
        with session.begin():
            claimed = PostgresJobRepository(session).claim_next(
                ...
                worker_id="scope-replies-runtime-worker",
                lease_seconds=120,
            )
    ...
    try:
        with session.begin():
            assert session.scalar(select(func.count()).select_from(provider_requests_table)) == 4
            assert (
        ...
        reply = comments["xhs-comment-reply-2"]
        assert reply["root_comment_id"] == "xhs-comment-root-1"
        assert reply["parent_comment_id"] == "xhs-comment-root-1"
