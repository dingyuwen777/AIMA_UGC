...
class _FakeMediaReader:
    ...
    def get_media(self, content_id, position):  # type: ignore[no-untyped-def]
        if position == 2:
            raise ContentMediaCacheUnavailable
        if position != 0:
...
def test_content_media_route_returns_same_origin_bytes_and_stays_out_of_openapi() -> None:
    ...
    assert response.headers["x-aima-media-state"] == "available"
...
def test_content_media_route_returns_visual_placeholder_when_source_is_unavailable() -> None:
    app = FastAPI()
    install_content_media_routes(app, media_reader=_FakeMediaReader())

    response = TestClient(app).get(f"/api/v1/contents/{uuid4()}/media/2")

    assert response.status_code == 200
    assert response.headers["content-type"] == "image/svg+xml"
    assert response.headers["x-aima-media-state"] == "unavailable"
    assert response.headers["cache-control"] == "private, max-age=60"
    assert "图片暂不可用" in response.text
