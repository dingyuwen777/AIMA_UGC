...
def test_request_audit_writer_persists_exact_cost_and_summarizes_run(tmp_path: Path) -> None:
...
def test_recalculation_writes_derived_report_without_overwriting_original_audit(
    ...
    assert summary.total_cost_amount == Decimal("0.0002375")
    report = json.loads(report_path.read_text(encoding="utf-8"))
    assert report["schema_version"] == "llm-cost-recalculation.v1"
    assert report["source_audit_sha256"]
    assert report["summary"]["total_cost_amount"] == "0.0002375"
