...
describe('voice plaza media preview projection', () => {
  it('只把有真实源 URL 的小红书图片投影到同源缓存地址', () => {
    ...
    expect(projected.media?.[1]?.preview_url).toBeNull()
    expect(projected.media?.[1]?.url).toBeNull()
    expect(projected.media?.[2]?.preview_url).toBe('https://example.invalid/cover')
  })
