...
test('AI irrelevant 内容可在真实声音广场人工纳入并进入默认业务列表', async ({ page }) => {
  ...
  await contentRow.getByRole('button', { name: '人工标记为相关' }).click()
...
test('AI relevant 内容可人工排除并撤销，恢复 AI 业务基线', async ({ page }) => {
  ...
  await contentRow.getByRole('button', { name: '人工标记为不相关' }).click()
  ...
  await contentRow.getByRole('button', { name: '撤销人工判断' }).click()
