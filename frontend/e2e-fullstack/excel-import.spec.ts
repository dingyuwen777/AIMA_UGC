...
async function configureRelevance(request: APIRequestContext, suffix: string): Promise<void> {
  const packResponse = await request.post('/api/v1/keyword-packs', {
    data: { name: `Stage8F Full-stack ${suffix} ${Date.now()}` },
  })
...
test('Excel 浏览器上传经过真实 API、Worker 和 PostgreSQL 后可在声音广场查看', async ({ page, request }) => {
  ...
  await configureRelevance(request, 'success')
  const created = await uploadExcel(page, fixturePath!)
...
test('结构合法但业务字段非法的 Excel 由真实 Worker 进入 failed 且页面准确解释终态', async ({ page, request }) => {
  ...
  await configureRelevance(request, 'failure')
  await uploadExcel(page, fixturePath!)
