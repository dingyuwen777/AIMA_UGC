...
test.beforeEach(async ({ page }) => {
  await page.route('**/api/v1/**', async (route) => {
    ...
    if (url.pathname === `/api/v1/import-batches/${batchId}/supplement-eligibility`) return route.fulfill({ contentType: 'application/json', body: JSON.stringify({ batch_id: batchId, targets: [{ platform: 'xiaohongshu', target_count: 1 }], diagnostics: [{ platform: 'xiaohongshu', direct_target_count: 1, resolution_candidate_count: 0, blocked_count: 0, block_reasons: {} }, { platform: 'weibo', direct_target_count: 0, resolution_candidate_count: 1, blocked_count: 0, block_reasons: { exact_resolution_unavailable: 1 } }, { platform: 'kuaishou', direct_target_count: 0, resolution_candidate_count: 0, blocked_count: 1, block_reasons: { identity_unavailable: 1 } }] }) })
...
test('creates a TikHub supplement Run only for a platform that exists in the Batch', async ({ page }) => {
  ...
  await expect(drawer.getByText(/有 1 条分享\/文章链接，当前缺少精确解析能力/)).toBeVisible()
