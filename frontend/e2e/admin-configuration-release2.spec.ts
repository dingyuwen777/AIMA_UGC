...
test('vehicle create and edit use product dialogs without exposing internal code', async ({ page }) => {
  await page.setViewportSize({ width: 1440, height: 900 })
  await mockAdmin(page)
  await page.goto('/admin/configuration')
...
test('brand creation never asks administrators for internal code', async ({ page }) => {
  await page.setViewportSize({ width: 1440, height: 900 })
  await mockAdmin(page)
  await page.goto('/admin/configuration')
  ...
  await expect(dialog.getByPlaceholder('请输入品牌名称')).toBeVisible()
  await dialog.getByRole('button', { name: '取消', exact: true }).click()

  await expect(page.getByText('品牌编码', { exact: true })).toHaveCount(0)
