...
test('从声音广场打开全局任务中心并统一查看三类活动任务', async ({ page }) => {
  ...
  await expect(page.getByText('AI 分析任务')).toBeVisible()

  await taskTrigger.click()
  const drawer = page.getByRole('complementary', { name: '任务中心' })
  await expect(drawer).toBeVisible()
  await expect(drawer.getByText('AI 分析任务 21', { exact: true })).toBeVisible()
  await expect(drawer.getByText('爱玛关键词采集', { exact: true })).toBeVisible()
  await expect(drawer.getByText('Excel 导出', { exact: true })).toBeVisible()
  await expect(drawer.getByText('3 个', { exact: true })).toBeVisible()
  await expect(drawer.getByText('主动采集 · 小红书', { exact: true })).toBeVisible()
