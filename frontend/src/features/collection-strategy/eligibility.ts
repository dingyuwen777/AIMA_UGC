...
export function planExecutionReason(input: PlanExecutionEligibilityInput): string | null {
  if (input.keywordPackIds.length === 0) return '请至少选择一个 Discovery 词包。'
  if (input.platforms.length === 0) return '请至少选择一个目标平台。'
  if (input.requireRelevance && !input.relevanceAvailable) return '全局 Relevance 尚未配置。'
  ...
  for (const packId of input.keywordPackIds) {
    ...
    if (!pack.enabled) return `Discovery 词包“${pack.name}”当前已停用。`
    details.push(pack)
  }
  ...
  for (const selection of input.platforms) {
    ...
    if (!provider) return `目标平台 ${selection.platform} 的 Provider 配置当前不可用。`
    ...
    if (!capability) return `目标平台 ${selection.platform} 当前不支持关键词发现。`
  }
