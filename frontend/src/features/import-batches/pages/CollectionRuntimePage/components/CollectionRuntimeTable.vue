  ...
  >
    ...
    >
      ...
      <div class="actions">
        ...
        <button
          v-if="item.record_type === 'excel_import' && item.import_batch_id && item.status === 'succeeded'"
          type="button"
...
.table-row { position: relative; min-height: 96px; padding: 13px 14px; border-bottom: 1px solid var(--aima-border); }
.table-row:last-child { border-bottom: 0; }
.identity strong, .identity span { display: block; }
.identity strong { overflow: hidden; color: #1f2737; text-overflow: ellipsis; white-space: nowrap; }
.identity span { margin-top: 6px; color: #6d7687; font-size: 11px; }
.type-pill { display: inline-block; padding: 5px 8px; border-radius: 5px; color: #12804b; background: #eaf8f1; font-size: 11px; }
.type-pill--tikhub_discovery { color: #6941c6; background: #f2edff; }
.type-pill--tikhub_batch_supplement { color: #b54708; background: #fff3e8; }
.progress-cell > div:first-child { display: flex; align-items: center; gap: 6px; color: #414a5b; font-size: 12px; }
...
.status--partial_success { background: var(--aima-warning); }
.status--failed { background: var(--aima-danger); }
.progress-track { width: 100px; height: 5px; margin-top: 9px; overflow: hidden; border-radius: 5px; background: #edf0f5; }
.progress-track span { display: block; height: 100%; border-radius: inherit; background: #2563eb; }
.stage-pill { display: inline-block; padding: 5px 8px; border-radius: 5px; color: #2563eb; background: #eef4ff; font-size: 11px; }
.stats-cell { display: flex; gap: 11px; }
.stats-cell span, .stats-cell strong { display: block; }
.stats-cell span { color: #737c8c; font-size: 10px; }
.stats-cell strong { margin-top: 4px; color: #263043; font-size: 12px; }
.related span, .related strong, .time-cell span { display: block; font-size: 11px; }
.related span, .time-cell span + span { color: #718096; }
.related strong { margin-top: 5px; color: #3f4859; font-weight: 500; }
.time-cell span { color: #3f4859; }
.time-cell span + span { margin-top: 6px; }
.actions { display: flex; flex-direction: column; align-items: flex-start; gap: 6px; }
.actions button { height: 29px; padding: 0 9px; border: 1px solid #ff85af; border-radius: 5px; color: var(--aima-primary); background: #fff; cursor: pointer; font-size: 11px; }
.row-error { grid-column: 1 / -1; margin-top: 10px; padding: 8px 10px; border: 1px solid #ffb8c5; border-radius: 5px; color: #d62f3a; background: #fff5f7; font-size: 11px; }
