...
import type { ImportBatchSummaryResponse } from '../../../../../generated/api/client'
import { formatNumber } from '../../../format'

defineProps<{ summary: ImportBatchSummaryResponse | null; loading: boolean }>()
</script>
...
<template>
  ...
  >
    ...
    <article class="kpi-card kpi-card--purple">
      <span class="kpi-icon">▤</span>
      <div><span>今日导入内容</span><strong>{{ loading && !summary ? '—' : formatNumber(summary?.rows_ingested_today) }}</strong></div>
    </article>
  </section>
</template>
