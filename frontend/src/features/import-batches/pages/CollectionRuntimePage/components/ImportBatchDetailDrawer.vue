...
const sourceRetentionText = computed(() => {
  if (sourceRetention.value.expiresAt === null) return '源数据文件在任务结束后继续保留 7 天；到期只清理文件内容，批次、入库数据和来源元数据继续保留。'
  if (sourceRetention.value.expired) return '源数据文件已超过 7 天保留期并进入自动清理；批次、入库数据和来源元数据继续保留。'
  return `源数据文件保留至 ${formatDateTime(sourceRetention.value.expiresAt)}；到期只清理文件内容，批次、入库数据和来源元数据继续保留。`
})
  ...
  >
    <template #header>
      <template v-if="item">
        ...
        >
          <button
            v-for="tab in [{ name: 'overview', label: '运行概览' }, { name: 'stages', label: '执行进度' }, { name: 'status', label: '任务信息' }, { name: 'errors', label: '问题记录' }]"
            :key="tab.name"
            type="button"
    ...
    >
      ...
      >
        ...
        <div class="fact-grid">
          <div><span>导入方式</span><strong>本地文件</strong></div>
...
.detail-tabs { display: grid; height: 44px; grid-template-columns: repeat(4, 1fr); padding: 0 10px; border-bottom: 1px solid var(--aima-border); background: var(--aima-surface); }
...
.detail-title h2 { min-width: 0; margin: 0; overflow: hidden; color: var(--aima-text); font-size: 18px; font-weight: 700; line-height: 26px; text-overflow: ellipsis; white-space: nowrap; }
.status-tag { flex: none; padding: 4px 10px; border-radius: 6px; color: var(--aima-color-info); background: var(--aima-color-bg-subtle); font-size: 12px; font-weight: 500; white-space: nowrap; }
.status-tag--succeeded { color: var(--aima-success); }
.status-tag--failed { color: var(--aima-danger); background: var(--aima-color-error-bg); }
.status-tag--cancelled { color: var(--aima-text-muted); }
...
.fact-grid strong { overflow: hidden; margin-top: 8px; color: var(--aima-text); font-size: 16px; font-weight: 700; line-height: 22px; text-overflow: ellipsis; white-space: nowrap; }
...
.stat-grid strong { margin-top: 4px; color: var(--aima-primary); font-size: 16px; font-weight: 700; line-height: 22px; }
...
.stage-row--current i { border-color: var(--aima-color-info); background: var(--aima-color-info); }
.stage-row--current b { color: var(--aima-color-info); }
...
.drawer-footer :deep(.aima-button.is-secondary) { min-width: 118px; }
