...
import ImportBatchFilters from './components/ImportBatchFilters.vue'
import ImportBatchTable from './components/ImportBatchTable.vue'
import ImportKpiCards from './components/ImportKpiCards.vue'
...
async function upload(file: File): Promise<void> {
  ...
  await store.openDetail(created.batch_id)
}
...
<template>
  <AppShell>
    <div class="page-header">
      <div><h1>采集运行中心</h1><p>统一查看 Excel 导入批次的处理进度、结果与异常</p></div>
    ...
    />
    <ImportBatchFilters
      v-model:identifier="store.filters.identifier"
    ...
    <div class="list-heading">
      <strong>导入批次记录</strong><span>创建时间：最新优先</span>
    </div>
    <ImportBatchTable
      :items="store.items"
      :loading="store.loading"
...
.page-actions button { height: 42px; padding: 0 19px; border: 1px solid #d7dce5; border-radius: 7px; color: #384153; background: #fff; cursor: pointer; }
...
.list-heading { display: flex; align-items: center; justify-content: space-between; margin: 23px 0 12px; }
...
.pagination { display: flex; align-items: center; justify-content: flex-end; gap: 20px; min-height: 72px; color: #717b8d; font-size: 13px; }
