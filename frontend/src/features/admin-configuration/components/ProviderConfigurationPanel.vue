  ...
  <div class="provider-layout">
    <section class="card provider-list">
      ...
      >
        ...
        <div
          v-else-if="archivedItems.length === 0"
          class="archived-state"
        >
          暂无已归档配置。
        </div>
    ...
    <section class="card provider-form">
      ...
      <div class="form-grid">
        <label><span>配置名称</span><input
          v-model="draft.displayName"
          placeholder="例如：默认 AI 模型"
        ></label>
        <label v-if="isLlm"><span>模型标识</span><input
          v-model="draft.model"
          placeholder="填写服务商提供的模型标识"
        ></label>
        <label class="span-2"><span>服务地址</span><input
          v-model="draft.baseUrl"
          :placeholder="isLlm ? 'https://provider.example/v1' : 'https://api.tikhub.dev'"
        ></label>
        <label class="span-2"><span>访问密钥</span><input
      ...
      <details class="advanced-settings">
        ...
        <div class="advanced-grid">
          <label><span>单次请求最长等待时间（秒）</span><input
            v-model.number="draft.timeoutSeconds"
            type="number"
            min="1"
            max="3600"
          ></label>
          <label><span>{{ isLlm ? '结果校验失败重试次数' : '请求失败重试次数' }}</span><input
            v-model.number="draft.maxRetries"
            type="number"
            min="0"
            max="20"
          ><small v-if="isLlm">仅在模型返回结果不符合系统要求时重试。</small></label>
          <label><span>同时请求数上限</span><input
            v-model.number="draft.maxConcurrency"
            type="number"
            min="1"
            max="5000"
          ><small>控制同一时间最多发起多少个请求；系统会据此自动安排任务分片。</small></label>
          <label><span>每秒请求启动上限</span><input
            v-model="draft.maxRps"
            type="number"
            min="1"
            max="10000"
            placeholder="留空表示不额外限速"
          ><small>服务商存在每秒请求限制时填写；留空表示不额外限速。</small></label>
        </div>
      </details>
      ...
      <div class="switches">
        <label class="check-row"><input
          ...
          type="checkbox"
        ><span><strong>启用配置</strong><small>停用后，新任务不会再选择这项配置。</small></span></label>
        <label
          v-if="isLlm"
          class="check-row"
        ><input
          ...
          type="checkbox"
        ><span><strong>设为默认 AI 模型</strong><small>没有特别指定时，新 AI 分析任务使用这项配置。</small></span></label>
      </div>

      <div class="security-note">
        <strong>密钥保护</strong><span>访问密钥不会在页面、审计记录或运行日志中显示明文；更新密钥也不会影响已经开始执行的任务。</span>
      </div>
      ...
      <details class="technical-details">
        ...
        <dl>
          ...
          <div v-if="selectedItem">
            <dt>配置标识</dt><dd>{{ selectedItem.id }}</dd>
          </div>
          <div v-if="selectedItem">
            <dt>配置修订号</dt><dd>{{ selectedItem.revision }}</dd>
          </div>
        </dl>
      </details>
...
@media (max-width: 760px) { .form-grid,.advanced-grid { grid-template-columns: 1fr; }.span-2 { grid-column: auto; } }
</style>
