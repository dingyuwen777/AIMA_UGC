...
基础与 Stage 2 的 Excel 接线已经完成；Stage 3 保持 planned，Stage 4 在 Stage 3 完成前
继续 blocked。当前事实仍以代码、Contract、Schema/Migration、测试和 Blueprint 为准。
本文只批准一个最小增量方向：
...
| Stage 3 TikHub 持久 Canonical | planned | Stage 1 + 当前 Filter 基线 | Discovery 最终 Filter 输入 Canonical 持久化 |
| Stage 4 Replay 与历史补齐 | blocked | Stage 2 + Stage 3 | 旧/新 Canonical Artifact 可重跑当前 Filter 并幂等入库 |
