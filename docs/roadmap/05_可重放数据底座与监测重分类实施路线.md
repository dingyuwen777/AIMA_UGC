...
- 状态：Active
- 目标：在已经完成的 Keyword Pack Search 与 Brand/Vehicle Filter 基线上，**不重构现有业务主链，只在 Mapper 与当前 Brand/Vehicle Filter 之间增加一层可长期复用的 Canonical JSONL.gz Artifact，并提供从该 Artifact 重新执行 Filter → Dedup → Content Owner 的正式 Replay 能力**。这样以后新增/扩展 Brand、Vehicle、Alias 或调整确定性 Resolver 时，可以直接重筛已经成功归一化的数据，不需要重新上传 Excel，也不需要对已有 TikHub Raw 再次付费请求 Provider。
- 退出条件：Excel 单文件 Import、Data Import Campaign、TikHub Discovery 在成功生成 Canonical 后、进入 Brand/Vehicle Filter 前，都能把成功归一化的数据持久为受 ArtifactService/ArtifactStore 管理的有界 JSONL.gz Artifact；Filter 消费该持久 Canonical；当前基线前后数据可以从现有 Source/Raw/历史 Chunk 补齐可重放 Canonical；正式 Replay 能冻结当前 BrandVehicleFilterSnapshot，按 Filter → Dedup → Content Owner 幂等补入新命中 Content；全链路来源、失败、容量和恢复有直接证据；长期知识同步后本 Roadmap 退出 live `docs/roadmap/`。
- 依赖：当前已经完成的 Canonical/Content Owner、ArtifactService/ArtifactStore、Data Import Campaign、Collection Provider Request/Attempt/Raw、Brand/Vehicle Catalog/Resolver/Filter 与 PostgreSQL Durable Job Runtime。完整 4000 万正式生产迁移仍受 [`docs/roadmap/03_4000万历史数据迁移实施方案.md`](03_4000万历史数据迁移实施方案.md) 的容量、生产授权和全量对账门禁约束。

本文是**已批准并按 Stage 推进的后续 Roadmap**。Stage 1 的共享 Canonical Artifact
基础、Stage 2 的 Excel 接线与 Stage 3 的 TikHub 接线已经完成；Stage 4 前置已满足并保持
planned，尚未实现 Replay 或历史补齐。当前事实仍以代码、Contract、Schema/Migration、测试和 Blueprint 为准。
本文只批准一个最小增量方向：
...
> 当前 Filter 第一次没命中的 Canonical，未来新增车型/品牌/Alias 后，怎样不用重新取得原始数据就能再筛一次？
...
### 2.3 单文件 Excel 已经持久化 Canonical Artifact

当前 [`backend/src/aima_ugc/bootstrap/import_worker.py`](../../backend/src/aima_ugc/bootstrap/import_worker.py) 的 v2 主链是：

```text
XLSX Artifact
→ convert_excel_to_canonical_jsonl()
→ CanonicalArtifactWriter 写入 JSONL.gz
→ 与 Processing Import Batch 原子绑定为唯一 linked Artifact
...
`contents.jsonl` 只作为 Mapping 阶段的临时写入文件；Filter 不再直接消费它。首次成功后，
Worker 只消费经过 metadata、SHA-256、byte size、gzip、JSON 和当前 Contract 完整预检的持久
Artifact；重试/接管按 Batch 恢复同一件 linked Canonical，不重新制造平行副本。

### 2.4 Historical/Data Import Campaign 已经使用 Pure Canonical Chunk

当前 [`backend/src/aima_ugc/adapters/providers/imports/historical_chunk.py`](../../backend/src/aima_ugc/adapters/providers/imports/historical_chunk.py)
只把成功 Mapper 的合法 `CanonicalContentV1` 写入有界 `chunk-XXXXXXXX.jsonl.gz`，不再把
`candidate/filtered` 业务结果写进 Artifact。Mapper invalid 的最小行事实单独保存在 Chunk
Item stats，消费阶段再与 filtered/matched 结果汇入既有逐行账本。
...
因此 **Canonical Artifact 不因当次 Brand/Vehicle Filter 结果而缺失数据**。

### 2.5 TikHub 已经先保存 Raw/Candidate

Collection 当前已经先持久化 Provider Request/Attempt/Raw/Candidate，再进入 Mapper，并把 TikHub Discovery 收敛为 Canonical 后统一执行 Brand/Vehicle Filter。

Roadmap 05 只把“最终送入当前 Brand/Vehicle Filter 的 Canonical”同时保存为可重放 Artifact；不改变 Provider Request/Attempt、Raw、Candidate、分页、费用和 Detail fallback 边界。
...
flowchart TD
    ...
    E --> R[Replay]
    R --> F2[新的 Brand/Vehicle Filter Snapshot]
...
## 4. 当前 Filter 对自有品牌和竞品的语义不变

Roadmap 05 **不改变当前 Brand/Vehicle Filter**。
...
仍属于**入库后的 Content 查询/专题研究条件**，不是 Roadmap 05 的 Canonical 保存规则，也不是当前来源 Search 条件。
...
保存所有**成功生成合法 CanonicalContentV1 的 observation**，不先经过当前 Brand/Vehicle Filter，也不先做业务 Dedup。
...
> **保存该 Candidate 本次实际送入最终 Brand/Vehicle Filter 的 Canonical。**
...
```mermaid
flowchart TD
    A[当前 Filter 基线] --> B[校准自有品牌 + 竞品 Brand/Vehicle/Alias]
    B --> C[导入过去一年数据]
    C --> D[当前 Filter → Content]
...
建议把实际需要研究的自有品牌和竞品配置为 active，并根据业务范围使用当前 `all_active` 或 `selected` Brand Scope。
...
+
任务创建时冻结的 BrandVehicleFilterSnapshot
```

不建立第二套 Filter Version 表；直接复用当前已经存在的冻结 Snapshot Contract。
...
每个 Stage 都是独立施工单元；从当时最新 `main` 开始，完成、验证、PR、merge 后再进入下一 Stage。中间 `main` 必须继续支持当前 Brand/Vehicle Filter 业务和并行的新 UI/专题功能。

| Stage | 当前状态 | 依赖 | 核心结果 |
| --- | --- | --- | --- |
| Stage 1 Canonical Artifact 基础 | completed | 当前 Brand/Vehicle Filter 基线已完成 | 共用 JSONL.gz Writer/Reader + Artifact/lineage 最小 Contract |
| Stage 2 Excel 持久 Canonical | completed | Stage 1 | 单文件 + Historical/Data Import 在 Filter 前持久 Canonical |
| Stage 3 TikHub 持久 Canonical | completed | Stage 1 + 当前 Filter 基线 | Discovery 最终 Filter 输入 Canonical 持久化 |
| Stage 4 Replay 与历史补齐 | planned | Stage 2 + Stage 3 | 旧/新 Canonical Artifact 可重跑当前 Filter 并幂等入库 |

`planned` 表示前置已经满足但尚未开工，不表示 Replay 或历史补齐已经实现。
...
已完成。单文件 Import 与 Historical/Data Import Campaign 都只在唯一 linked Pure Canonical
Artifact 完整建立并通过共享 Reader 预检后执行 Filter；精确运行事实由当前代码、Migration、
测试及 Blueprint 维护。以下内容保留本 Stage 的目标、边界和退出条件。
...
- 按已批准的 clean break 不保留旧 outcome Chunk / v1 Payload 兼容；Migration 发现仍活跃或可重试旧存量时失败关闭，新 Worker 不会错读；
...
已完成。TikHub Discovery 先确定一个 Search Provider Attempt 页面内每条 Candidate 实际用于
最终 Filter 的 Search-derived 或 Detail-derived Canonical，再建立唯一 linked JSONL.gz
Artifact；共享 Reader 完整预检后，现有 Filter/Ingestion 才消费这些记录。Artifact 父级绑定
页面 Search Attempt，逐行 Source 继续保留实际 Search/Detail Attempt、Raw 与 item locator；
filtered 和重复 identity 的合法 observation 不丢失，Mapper invalid 仍只进入既有
Raw/Candidate/error ledger。精确实现、Migration 和验证事实由当前代码、Schema、测试、
Blueprint 与已归档 Change 维护。以下内容保留本 Stage 的目标、边界和退出条件。

## 目标

在当前 TikHub Discovery 主链中加入同一 Persistent Canonical Artifact 能力。
...
3. 不改变当前 Search→Detail fallback 判断；
...
让已有 Canonical Artifact 成为正式可重放输入，并把当前 Filter 基线完成后、Roadmap 05 完成前已经导入/采集的数据补齐到该结构。
...
→ 冻结当前 BrandVehicleFilterSnapshot
...
当前 Brand/Vehicle Filter 基线完成以后，可以同时开发：
...
该功能可以立即基于 Content Brand/Vehicle Evidence 独立开发，不需要等待 Roadmap 05。
...
flowchart TD
    ...
    B -->|Brand/Vehicle/Filter 变化时重筛| C[当前 Brand/Vehicle Filter]
    C --> D[Formal Content]
...
1. 重新读取本 Roadmap、当前 Brand/Vehicle Filter 机器事实和最新 `main`；
...
5. 保持当前 Brand/Vehicle Filter、Content identity、Content Owner、Provider Raw/Attempt 语义不变，除非当前 Stage 明确要求并已有上游批准；
...
要求：
- 以当前最新 main、Roadmap 05 当前 Stage、当前 Brand/Vehicle Filter 机器事实和真实代码为 Requirement Source；
- 只实现当前 Stage；
- 采用现有 Raw/Source → Mapper → Canonical → Brand/Vehicle Filter → Dedup → Content Owner 主链；
...
1. 单文件 Excel：Source → Mapper → Persistent Canonical → Brand/Vehicle Filter → Dedup → Content；
