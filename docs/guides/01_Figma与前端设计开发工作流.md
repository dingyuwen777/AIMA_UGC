...
报告策略当前已接通真实双 XLSX 上传、管理员 Report Job/API、进度轮询和结果展示；Figma 中提交中、同步失败和成功状态由真实 Job 状态驱动。开发默认 Dry Run，只执行报告/LLM流程而不写飞书；前端不得通过延时、永久 Mock、假任务或假链接宣称成功。更细的节点、Owner 和响应式验收见 [`docs/guides/02_管理员配置Figma开发基线.md`](02_管理员配置Figma开发基线.md)。
