...
用户随后补充的微博 `ttarticle` 链接与上述标准帖子是两个不同身份。对标准帖子再发 1 次 Detail、计划费用 0.001 美元，响应可提取 1 条内容，但不包含所给文章 ID；现有证据不能证明该帖是文章唯一原始父帖。用户又提供一条页面上显示 1 条评论的长文章 `2309405337665377009857` 作为正向样本，进一步明确允许把文章页面评论按产品口径计入“父帖评论”。当前导入只保存 `ttarticle_id`，正式 Runtime 在发出 Provider 评论请求前以 `identity_unavailable` 拒绝；TikHub 当前 [官方 SDK 接口参考](https://github.com/TikHub/TikHub-API-Python-SDK/blob/main/docs/reference.md)和[微博 Web V2 评论接口文档](https://docs.tikhub.io/381269409e0)只列出按帖子 ID 获取微博评论，未提供按文章 ID 直接取页面评论或唯一父帖的正式接口。对该正向样本的完整文章 ID 和去掉 `230940` 前缀后的数字分别请求 App Detail/Comments，共 4 次请求、计划费用 0.004 美元，4 次均为 HTTP 400，未获得可映射内容或评论。文章页面计数可作为待补采目标，不能作为已入库评论或线程身份的证据；若未来取得正式页面评论 Operation，须保存真实线程类型与精确归属，不伪装成帖子 `status_id`。本轮没有把短链解析升级为正式 Capability。上述价格是当前仓库 `pricing.toml` 的请求前估算，最终账单未核验；五平台多页、恢复、短链和真实全栈仍待验证。
