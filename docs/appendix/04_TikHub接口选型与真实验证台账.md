...
用户随后补充的微博 `ttarticle` 链接与上述标准帖子是两个不同身份。对标准帖子再发 1 次 Detail、计划费用 0.001 美元，响应可提取 1 条内容，但不包含所给文章 ID；现有证据不能证明该帖是文章唯一原始父帖，文章“没有评论”也不能替代父帖身份解析。用户又提供一条页面上显示 1 条评论的长文章作为正向样本：当前导入只保存 `ttarticle_id`，正式 Runtime 在发出 Provider 评论请求前以 `identity_unavailable` 拒绝；TikHub 当前 [官方 SDK 接口参考](https://github.com/TikHub/TikHub-API-Python-SDK/blob/main/docs/reference.md)只列出按帖子 `status_id`/`post_id` 获取微博评论及按 `uid` 获取用户文章列表，未提供按文章 ID 直接取评论或唯一父帖的正式接口。用户确认继续遵守路线图的“唯一原始父微博评论”语义，因此不能把页面所示的 1 条文章评论直接计入本产品的父帖 Coverage。本轮没有把短链解析升级为正式 Capability。上述价格是当前仓库 `pricing.toml` 的请求前估算，最终账单未核验；五平台多页、恢复、短链和真实全栈仍待验证。
