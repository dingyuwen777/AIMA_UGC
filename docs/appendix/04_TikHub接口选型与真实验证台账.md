...
用户随后补充的微博 `ttarticle` 链接与上述标准帖子是两个不同身份。对标准帖子再发 1 次 Detail、计划费用 0.001 美元，响应可提取 1 条内容，但不包含所给文章 ID，不能证明它是文章原始父帖。用户又提供页面显示 1 条评论的长文章 `2309405337665377009857`。对该样本完整文章 ID 和去掉 `230940` 前缀后的数字分别请求 App Detail/Comments，共 4 次请求、计划费用 0.004 美元，4 次均为 HTTP 400，未获得可映射内容或评论。TikHub 当前 [官方 SDK 接口参考](https://github.com/TikHub/TikHub-API-Python-SDK/blob/main/docs/reference.md)和[微博 Web V2 评论接口文档](https://docs.tikhub.io/381269409e0)只列出按帖子 ID 获取微博评论，未提供按文章 ID 直接取页面评论的正式接口。用户最终决定本轮不对 `ttarticle` 长文章补采评论；该决定取代此前讨论的文章页面评论或原始父帖映射方案。新增的文章 ID 提取逻辑已移除，历史 `ttarticle_id` 定位字段只作为阻断条件，不发微博帖子评论请求。页面显示评论数不等于已采集数。本轮没有把短链解析升级为正式 Capability。上述价格是当前仓库 `pricing.toml` 的请求前估算，最终账单未核验；该阶段的多页、恢复、短链和真实全栈仍待验证；后续证据见下文。

同日追加真实结构核对：上述可见快手作品一级评论前两页分别返回 30、20 条，全部经生产 Mapper 挂到同一内容且无跨页重复，第二页仍有游标；一条报告 6 条回复的根评论取回 6 条，下一页为空并终止。5 次请求、计划 0.005 美元，只证明该线程回复数相符和一级评论前两页可用。小红书 [App V2 图文详情](https://docs.tikhub.io/420136391e0) 使用 `share_text` 时，一个已验证长链接返回相同 `note_id`，一个近期公开短链返回唯一 typed `note_id`；旧公开“爱玛”短链两次 HTTP 400。实际 4 次详情请求、按仓库定价计划 0.040 美元；一次计划的额外评论请求被费用上限发送前拒绝。

用 TikHub 官方 `get_endpoint_info` 现场核价：[抖音 App V3 按分享链接获取作品](https://docs.tikhub.io/186826220e0)为 0.001 美元/次，[快手 Web 按 URL 获取作品](https://docs.tikhub.io/188317971e0)为 0.002 美元/次，[快手 Web 生成短链](https://docs.tikhub.io/343506805e0)为 0.001 美元/次。抖音官方示例短链 1 次请求返回唯一可由生产 Mapper 映射的数字 `aweme_id`。快手已验证作品的 3 组生成→解析回环中，生成响应 `shareObjectId` 与输入 `photo_id` 均相同，但按 URL 查询返回的 `data.photo.photoId` 均不同；6 次业务请求、计划 0.009 美元。因此快手该候选不能作为同一内容的精确身份映射，维持阻断。抖音 App V3 分享链接详情的已核价格已写入生产 `pricing.toml`；小红书和抖音短链接入正式解析 Operation/Attempt/Raw，隔离 PostgreSQL 已验证其 typed ID 来源和评论挂载，小红书评论 503 重试复用已完成的解析 Raw。快手两个候选接口的价格没有写入生产价格表，也没有接入自动解析。查价请求与实际账单费用不在上述业务请求估算内。

追加微博/B站候选：按官方 `get_endpoint_info`，微博 App `fetch_video_detail` 和 B站 Web `fetch_one_video_v3` 都是 0.001 美元/次。微博 [视频详情文档](https://docs.tikhub.io/410358107e0)链接示例中的 `1034:5232127105761312` 及其去前缀数字各一次均 HTTP 400；文档另一数字示例 `5242977759006596` 两次 HTTP/业务码 200，实际响应为 `data.status.idstr` 数字，不是文档写的 `items[0].data.idstr`。B站 [Web V3 URL 详情](https://docs.tikhub.io/321173063e0)对一个公开 `b23.tv` 短链返回 HTTP 400。这些样本不能证明视频短链与原内容一一对应，因此没有接入正式解析或评论请求。微博 4 次候选请求计划 0.004 美元，B站 1 次计划 0.001 美元，未核最终账单。

同日继续使用五个平台已确认的原生 ID 做有界真实分页探测，全部通过生产 Operation/Transport/Extractor/Mapper，未保存正文、账号或 Raw。快手可见“爱玛”作品一级评论在 12 页返回 `30+9×20+11+0=221` 条不重复根评论，最后一页以 `empty_page` 终止，12 次请求计划费用 0.012 美元；这补足先前只有前两页的证据。固定公开样本中，小红书 1 页 3 条、微博 1 页 2 条、B站 1 页 2 条均以 `provider_exhausted` 终止；抖音 2 页分别返回 20、5 条，其中第二页 5 条与第一页重复，实际唯一评论为 20 条，也以 `provider_exhausted` 终止。四平台共 5 次请求、计划费用 0.014 美元；重复 ID 必须由持久补采按评论身份去重，不能把原始返回 25 条写成 25 条新增评论。

对同组四平台公开样本额外做有界回复探测，6 次请求（4 次根评论首页、2 次回复），计划费用 0.015 美元：抖音一条报告 3 条回复的根评论取回 3 条，B站一条报告 1 条回复的根评论取回 1 条，均由生产 Mapper 正确挂到原内容与根评论并以 `provider_exhausted` 终止，未见重复。小红书与微博固定样本的本轮首页未找到报告有回复的根评论，因此这两个**具体样本**无法提供实时回复正例；现有历史真实字段 Fixture 与隔离 PostgreSQL 五平台回复集成仍承担其结构回归。上述三次新探测合计 23 次请求、计划费用 0.041 美元，不等于最终账单。真实 Probe 不证明用户不可见、删除或平台未返回的评论存在。
