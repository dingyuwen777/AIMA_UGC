...
Local ArtifactStore  默认 Raw/输入/导出/报告文件存储
...
完整离线生产 Release 和 PostgreSQL + Artifact 协调 Backup/Restore 当前尚未闭环，不能把长期方向写成当前已经完成。
...
- 调试入口复用生产代码；
- 默认一个 PostgreSQL；
- API、Worker、Scheduler、Migration 复用同一后端代码和镜像基础；
- 配置显式，不让底层猜；
- 外部 ID、时间、状态、错误语义统一；
