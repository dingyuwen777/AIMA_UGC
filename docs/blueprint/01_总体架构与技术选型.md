...
其中 [`compose.yaml`](../../compose.yaml) 是 Linux / WSL / 公司服务器共用的 canonical Runtime；[`compose.windows.yaml`](../../compose.windows.yaml) 只覆盖 Windows Docker Desktop 的持久存储 source，不复制 API/Worker/Scheduler/Migration/Health/网络等业务 Runtime。源码开发仍使用 `env.local + scripts/dev/backend.py + scripts/dev/frontend.py`，与完整 Compose Runtime 是两种运行入口，不是两套业务实现。
...
GitHub 离线 Release Workflow 已建立当前基础，但完整状态和实施顺序仍以 [`docs/roadmap/02_生产上线实施路线.md`](../roadmap/02_生产上线实施路线.md) 与 [`docs/appendix/11_生产部署与离线Release方案.md`](../appendix/11_生产部署与离线Release方案.md) 为准。
...
源码开发时前端由 Vite 提供；完整容器 Runtime 中，[`Dockerfile`](../../Dockerfile) 构建 Vue 静态资源并由非 root Nginx Runtime 提供，同时同源代理 `/api` 与 `/health`。这两种入口复用同一前后端业务实现，不改变下面四个正式 Python 进程的职责边界。
...
- [`backend/src/aima_ugc/entrypoints/worker_main.py`](../../backend/src/aima_ugc/entrypoints/worker_main.py)
...
| `collection` | Plan、Occurrence、Run、Scope、Provider Request/Attempt、Candidate、Decision | [`backend/src/aima_ugc/modules/collection/README.md`](../../backend/src/aima_ugc/modules/collection/README.md) |
| `content` | Canonical Ingestion、Content/Comment Current、Version、Metric、Query | [`backend/src/aima_ugc/modules/content/README.md`](../../backend/src/aima_ugc/modules/content/README.md)、[`backend/src/aima_ugc/modules/content/ingestion.py`](../../backend/src/aima_ugc/modules/content/ingestion.py) |
| `ingestion` | Excel Import Batch、Import Job、文件入口父事实 | [`backend/src/aima_ugc/modules/ingestion/README.md`](../../backend/src/aima_ugc/modules/ingestion/README.md) |
| `analysis` | AI relevance、voice_type、sentiment、labels、Analysis Request/Result | [`backend/src/aima_ugc/modules/analysis/README.md`](../../backend/src/aima_ugc/modules/analysis/README.md) |
...
这只是长期导航所需的主项，不是完整根目录镜像。容器入口的机器事实直接看根 [`Dockerfile`](../../Dockerfile)、[`compose.yaml`](../../compose.yaml)、Windows storage-only override 与 [`env.production.example`](../../env.production.example)；完整 Production 是否可 Go-Live 仍由 Roadmap/Release Appendix 判定，不能从“文件已存在”反推出生产验收已完成。
...
- [`backend/src/aima_ugc/bootstrap/collection_scope.py`](../../backend/src/aima_ugc/bootstrap/collection_scope.py)：把 TikHub Transport、Raw Artifact、Candidate、Mapper、Ingestion 串起来
- [`backend/src/aima_ugc/bootstrap/content_http.py`](../../backend/src/aima_ugc/bootstrap/content_http.py)：把 Content Query Repository 和 Analysis Identity 组装成声音广场 HTTP Service
- [`backend/src/aima_ugc/bootstrap/analysis_concurrent_worker.py`](../../backend/src/aima_ugc/bootstrap/analysis_concurrent_worker.py)：把 Analysis Request、LLM、Validator、Persistence 串成正式 Job Executor
...
- [`backend/src/aima_ugc/modules/content/ingestion.py`](../../backend/src/aima_ugc/modules/content/ingestion.py)：Canonical 摄取领域入口
- [`backend/src/aima_ugc/modules/collection/scheduler.py`](../../backend/src/aima_ugc/modules/collection/scheduler.py)：Cron / latest_only 领域算法
- [`backend/src/aima_ugc/modules/analysis/content_labeling.py`](../../backend/src/aima_ugc/modules/analysis/content_labeling.py)：AI 请求、结构校验、Validation Retry
...
- [`frontend/src/features/admin-configuration/`](../../frontend/src/features/admin-configuration/)
- [`frontend/src/features/identity/`](../../frontend/src/features/identity/)
