...
Vue + Vite           当前前端开发/联调运行方式
Asia/Shanghai        业务调度和人工日志时区
UTC timestamptz      数据库存储和机器比较
```

已批准但**尚未进入当前仓库机器事实**的生产目标：

```text
Docker / Compose     Stage 11A 待实现的生产装配
Nginx                Stage 11A 目标中的前端静态 Runtime / 反向代理
Offline Release      Stage 11B 待实现
Coordinated Backup   Stage 11C 待实现
```

当前仓库根没有 `Dockerfile`、`compose.yaml`、`compose.production.yaml` 或 `env.production.example`。完整状态和实施顺序以 [`../roadmap/02_生产上线实施路线.md`](../roadmap/02_生产上线实施路线.md) 为准，不能把长期部署方向写成当前已经存在的运行文件。
...
当前本地前端由 Vite 提供；未来生产 Stage 11 的已批准目标是构建静态前端并由 Nginx Runtime 提供，不把该目标图误写为当前已有 Docker/Nginx 部署。
...
Migration 不放进 API 启动命令，避免多 API 副本同时升级数据库。
...
当前根目录**没有** `Dockerfile` 或 `compose*.yaml`；这些文件属于 Roadmap Stage 11A 的待实现目标。目录事实应以当前分支根目录为准，本文只提供导航，不维护第二份完整文件清单。
...
- 调试入口复用生产代码；
- 默认一个 PostgreSQL；
- API、Worker、Scheduler、Migration 复用同一后端代码；Stage 11 生产目标再让它们通过同一应用镜像的不同 command 运行；
- 配置显式，不让底层猜；
- 外部 ID、时间、状态、错误语义统一；
