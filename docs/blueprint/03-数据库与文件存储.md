...
`billing_status` 至少区分 `not_billable`、`estimated`、`confirmed` 和 `unknown`。billable Attempt 在创建时保存发送前实际采用的 `unit_price_snapshot/estimated_cost/currency/unit`；Dispatcher 必须把这份持久化计划快照交给 Provider Client。确定响应没有权威逐请求账单时，不能把本地估算伪装成 `actual_cost`，也不能用响应缺少 Billing 把计划费用覆盖成不计费；只有 Provider 给出权威逐请求费用时才使用 `confirmed + actual_cost`。预算预留提交后，Dispatcher 在短事务中沿 Attempt→Request→Scope→Run 锁住所属 Job，验证当前 `status=running + lease_token + lease/deadline 未过期`，再以 CAS 将 `reserved → dispatching`；只有赢得 CAS 的执行者可以调用 Provider Client。Transport 层必须关闭会在一次调用内部悄悄重发网络请求的自动重试；同一 Attempt 不能再次进入发送，任何真正重发都先建新 Attempt。活跃 Transport 明确证明请求未进入发送边界时，Dispatcher 可把已进入数据库 `dispatching` 的 Attempt 终结为 `not_sent` 并清除发送开始时间；一旦进程崩溃、超时或无法取得完整性已验证 Raw，就不能事后宣称未发送，而要保守转为 `unknown`，保留预留且禁止复用该 Attempt 发送。Collection Attempt Reconciler 在所属 Job Lease 丢失/Deadline 到达后锁住遗留 `dispatching` Attempt：先校验确定性路径上已落盘的 Raw，存在时恢复 terminal 状态，不存在或损坏时才转为 `unknown`；旧 Worker 受 Job Fencing 约束，不能随后覆盖。后续真实网络重试创建新 Attempt，并把 `potential_duplicate_charge` 标记为真。逻辑请求的 `attempt_count` 和已知费用按 Attempt 汇总，Request 状态镜像最新 Attempt；创建新 Attempt 时重置为 `pending`。Canonical、版本和指标记录引用实际产生数据的 Attempt，从而保留完整来源。为保持该来源链，数据库 Trigger 必须禁止：Scope 已有 Provider Request 后修改其 `run_id/platform/source` 身份；Provider Request 已有 Attempt 后修改 `scope_id/provider_config_id/provider/operation`；Attempt 修改 `provider_request_id`，或在 terminal 一次性设置 `raw_artifact_id` 后再次改写/清除。后续计费对账只能修改计费状态/金额等允许字段，不能改来源身份。
...
Stage 7 `20260815_0012` 已建立最终 Provider 多级预算模型。预算账户绑定稳定 Provider Config，不能只按 Provider 类型混用额度：
...
provider_budget_accounts
  ...
  check(scope_type in ('global', 'run', 'run_comments', 'content_comments'))
  ...
  unique(provider_config_id, scope_key, period_start, dimension, unit)
  exclude using gist (
...
`scope_key` 固定为 `global`、`run:<uuid>`、`run_comments:<uuid>` 或 `content_comments:<uuid>`，并与互斥 `run_id/content_id` 身份保持一致。`btree_gist` 排斥约束禁止同一 Provider Config、Scope、维度、单位出现重叠预算周期；Migration 只在缺失时创建扩展，downgrade 不删除可能被其他对象共享的扩展。

每个 billable HTTP Attempt 在发送前必须取得完整 Reservation：普通 Operation 同时预留 `global + run`，评论/二级评论同时预留 `global + run + run_comments + content_comments`；每层都包含独立 `request_count` 与 `monetary_cost` 维度。Repository 按稳定账户 ID 顺序使用行锁，在同一事务检查 `settled + reserved + unknown + 本次预留 <= limit` 并全部预留；任一账户缺失、周期不覆盖、Config/Run 来源不一致或任一维超额，都整体回滚并在 Transport 前关闭失败。同一 Attempt 只有预留事务重放可通过唯一键幂等复用；真正网络重试必须创建新 Attempt 和新 Reservation。

货币 Reservation 使用发送前已经持久化的保守 `estimated_cost`，其来源由对应 Provider Adapter 的版本化 Pricing 负责。TikHub 只有目标 endpoint 的官方精确基础价已核验后才允许生成该快照；全局“多数接口价格”、历史平均费用或阶梯折扣推算不能替代未核验 endpoint 的硬预算价格。

终态结算语义：

- `completed + confirmed`：只有 Provider 给出权威逐请求费用时，货币 Reservation 才按 `actual_cost` 结算；实际费用高于预留也必须如实记录，账户进入超额后阻断后续发送；
- `completed + estimated`：Provider 没有权威逐请求费用，Attempt 的 `actual_cost` 保持 0，Budget 把原保守 Reservation 转为 `settled_amount`；该值表示预算占用上界，不得冒充 Provider 实际账单消费；
- `not_sent`：确认未发送，释放全部 Reservation；
- `unknown`：原 Reservation 转入 `unknown_amount` 并继续占用，直到独立 reconciliation 能证明应如何处理。

预算账户提供从 Reservation 重算 `reserved/settled/unknown` 的审计入口；聚合值漂移时明确失败，不自动改账掩盖问题。
...
所有 Schema 演进使用根目录 `alembic.ini` + `migrations/`。当前 Alembic 链从 `20260813_0001` 演进到 `20260815_0012`；`0012` 以 `20260815_0011` 为直接父 Revision，为 `provider_requests` 增加兼容可空的稳定 `provider_config_id` 外键并建立 `provider_budget_accounts/provider_budget_reservations` 最终多级预算 Ledger；历史 Revision 不改写。
