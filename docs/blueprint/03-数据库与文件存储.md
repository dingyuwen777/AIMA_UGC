...
Stage 7 billable Attempt 在 `reserved` 状态使用发送前确定的 Billing 快照。TikHub 只有目标 endpoint 已由后端 Pricing 配置核验后才允许构造 `billing_status='estimated'`，并保存保守 `unit_price_snapshot/estimated_cost/currency/unit`；此时 `actual_cost=0`，不能把估算值伪装成实际费用。不计费 Fake/文件型路径继续使用 `not_billable`。billable Attempt 在发送前还必须存在完整 `provider_budget_reservations`；缺少已核验价格、任何必需账户或 Reservation 时均关闭失败，Transport 不得被调用。

ProviderClient 发送时使用数据库中该 Attempt 已冻结的计划 Billing。若 Provider 的确定响应没有逐请求权威费用字段，计划快照继续保留为 `estimated` 且 `actual_cost=0`；只有 Provider 真正提供权威逐请求费用时才允许升级为 `confirmed` 并保存非零 `actual_cost`。后续 Pricing 变更不得重算历史 Attempt。
...
- `completed + confirmed`：请求次数结算为 1；仅当 Provider 提供权威逐请求费用时，货币维度按该 `actual_cost` 结算。实际费用高于发送前预留仍必须如实写入 `settled_amount`，不能截断为预算上限，后续请求由超额状态阻断；
- `completed + estimated`：Provider 没有权威逐请求费用。Attempt 的 `actual_cost` 必须保持 0；货币 Budget 把原保守 Reservation 转为 `settled_amount`。这里的 `settled_amount` 表示预算占用上界，不是 Provider 财务账单实际消费；
...
TikHub 当前普通业务响应不提供本系统已确认的逐请求权威费用，因此不能把 Pricing 估算或测试值写成 `actual_cost`。TikHub endpoint 基础价由后端版本化 Pricing 配置在发送前形成 Attempt 快照；未取得官方精确价格的 endpoint 关闭失败。价格配置不进入前端，也不保存 Secret。后续如实现 TikHub daily usage、余额或账单 reconciliation，必须作为独立事实能力增加。
