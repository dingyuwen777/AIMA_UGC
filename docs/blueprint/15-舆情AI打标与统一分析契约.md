...
只修改 `content_labeling_v1.md`，无需同步修改 Python 标签枚举。
...
每条结果恰好 1 个 primary_label
每条结果恰好 1 个 secondary_label
三个字段都是非空字符串
sentiment 必须属于当前 Prompt Taxonomy
primary_label 必须属于当前 Prompt Taxonomy
...
“标签不硬编码”不等于“相信模型自由输出”。最终写入系统的值仍然只能来自当前 Markdown 中的闭集。
...
首版每条内容只输出**一个主一级 + 一个主二级**。

1. 以作者主要诉求、主要评价对象、主要篇幅为先；
2. 具体产品/服务事实优先于泛品牌印象；
3. 明确故障优先“耐用性与质量 / 故障问题与稳定性”，但电池、智能功能、骑行性能有明确专属维度时优先专属标签；
4. 电池续航、衰减、充电、安全优先“电池、续航与充电”；
5. App/NFC/仪表/电子功能优先“智能化与电子功能”；
6. 售后维修、保修、配件、投诉优先“售后服务”；购买前、下单、提车优先“销售与购买体验”；
7. 具体车型价格/配置、优惠、养护成本优先“价格与价值”；品牌层面的品牌溢价才用“品牌评价 / 性价比与溢价”；
8. 广告、代言、直播、达人、联名等传播内容用“品牌评价 / 营销与传播”；
9. 多主题仍只选最核心主题，不输出数组；未来多标签属于独立 Contract 变更。
...
`ContentLabelAnalysisV1` 中具体标签字段使用普通字符串，不用具体标签 `Literal/Enum`：
...
未来数据库不建议把当前标签做 PostgreSQL ENUM，否则只改 Prompt 无法生效。
...
写入前已按对应 Prompt Taxonomy 验证。历史结果保留 Hash，后来修改标签体系不会抹掉旧结果当时规则。

正式表、唯一约束、attempt/费用审计、历史策略和 Migration 在未来 Analysis 数据库阶段单独冻结；P1 不建立表。
...
PromptTaxonomyLoader
ContentLabelAnalysisV1（固定结构，不硬编码业务标签枚举）
