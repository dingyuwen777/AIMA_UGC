...
- Worker 持久 Job 的精确注册以 `backend/src/aima_ugc/bootstrap/worker.py` 为机器事实；当前正式架构文档已经同步八种 Job，不再保留旧四项摘要。
...
| 用户可见行为/前后端/Full-stack/Provider 测试分层 | `docs/blueprint/06_开发约束与分阶段实施.md` + 按上方项目研发治理入口取得当前分层验证规则 |
