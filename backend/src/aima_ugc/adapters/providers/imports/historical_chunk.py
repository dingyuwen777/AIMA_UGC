...
def convert_historical_excel_to_chunks(
    *,
    ...
    """只执行 Reader/Mapper；逐块有界保留 Canonical 与 invalid 最小事实。"""
    ...
    contents: list[CanonicalContentV1] = []
    chunk_open = False

    def open_chunk(row_number: int) -> None:
        nonlocal chunk_open, descriptor_values, canonical_row_ordinals, invalid_rows, contents
    ...
    for row in iter_excel_rows(input_path, profile=profile):
        ...
        try:
            ...
            invalid_rows.append(HistoricalInvalidRow(row.row_number, "canonical_validation_error"))
            counters["invalid"] += 1
