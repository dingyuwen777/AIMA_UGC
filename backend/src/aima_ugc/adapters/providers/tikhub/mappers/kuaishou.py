"""TikHub 快手 Search V2 Raw → Canonical 纯 Mapper。"""
...
def map_content(
    ...
    """把真实 App Search V2 mixFeeds.feed 映射为内容 Observation。"""
    item = first_dict(raw, "feed")
    if not item:
        raise ValueError("快手 Search item 缺少 feed")
    ...
    author, author_fields = _map_author(item)
...
__all__ = ["KuaishouMappingContext", "map_content"]
