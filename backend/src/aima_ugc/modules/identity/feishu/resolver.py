...
会话撤销或数据库中的会话角色被受控更新后，下一次请求必须立即看到新状态；若把角色
塞进 Cookie 或进程内缓存，这些本地变更就会被旧快照遮住。每次按令牌 hash 读主键，
代价可接受，也让主动登出/撤销即时生效。

⚠️ 当前版本**不会在会话存续期自动回查飞书用户组**。飞书侧远端移组/撤权最迟在
当前会话到期后重新登录时生效（默认 8 小时）；不能把“每次读本地数据库”误写成
“远端权限即时同步”。
