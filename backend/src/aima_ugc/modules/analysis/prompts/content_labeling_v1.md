...
`item_no` 只用于当前批次请求/响应配对，不是业务字段。不要要求或推断内容 ID、平台、Provider、URL、互动指标、粉丝数、命中关键词、Raw 定位、源 Excel 情感或其他未提供字段。
...
2. 每条恰好一个 `sentiment`、一个 `primary_label`、一个 `secondary_label`；
3. 不得输出额外字段；
4. 三个标签必须使用下面机器 Taxonomy 中的完整原名，不得输出数组、空字符串、近义词或自造标签；
5. `secondary_label` 必须属于所选 `primary_label`；
6. 信息不足时仍按下方规则选择最合理的单一主标签；情感信息不足时使用“中性”。
...
7. 具体车型价格/配置、优惠、养护成本优先“价格与价值”；品牌层面的品牌溢价才使用“品牌评价 / 性价比与溢价”；
8. 广告、代言、直播、达人、联名等传播内容使用“品牌评价 / 营销与传播”；
9. 多主题仍只选择最核心主题，不输出数组。
...
- 每条三个标签都是机器 Taxonomy 中的当前值；
- 二级标签属于所选一级标签；
- 没有额外字段、解释、Markdown 或数组标签；
- 没有用典型表达做简单关键词匹配替代语义判断。
