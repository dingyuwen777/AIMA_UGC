...
- [`../../adapters/providers/imports_test/`](../../adapters/providers/imports_test/)：人工/离线入口；DeepSeek 示例仍可配置 250。`label_sentiment()` 直接读取本地 [`env.local`](../../../../../env.local) 和 Git Prompt，调用共用的并发核心并写入 JSONL/Excel，不初始化数据库；`WRITE_TO_DATABASE=False` 时整条离线流程保持无数据库运行。
