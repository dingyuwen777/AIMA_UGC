"""Collection 模块拥有的 Plan/Run/Scope 与 Provider Request/Attempt 表。"""
...
collection_plan_brands_table = Table(
    "collection_plan_brands",
    metadata,
    Column("plan_id", Uuid(), ForeignKey("collection_plans.id"), primary_key=True),
    Column("brand_id", Uuid(), ForeignKey("vehicle_brands.id"), primary_key=True),
    info={"owner": "collection"},
)
