...
- [ ] V4 只接收既有五个业务字段，明确忽略内容中的 Prompt Injection 指令，并按 relevance → source → intent → voice → targeted sentiment → evidence-backed labels 顺序判定。
- [ ] V4 增加“个人交易发声”，普通消费者个人交易不能与“真实用户发声”同时成立；二手经营主体仍归行业/渠道语义。
- [ ] Validator 拒绝伪造证据、主体/意图/voice 冲突、relevance/sentiment/labels 冲突及 `needs_judge` 主判结果，不使用中文关键词硬编码重新分类。
- [ ] 语义冲突或不确定时只对未解决条目发起 Judge；合法明确条目保持单次调用，Judge 仍只看到五个业务字段并沿用既有次数、停止、费用与审计边界。
- [ ] 既有 V3 Scheme 输出仍可校验和执行；新空库 bootstrap V4；现有数据库不会被部署静默切换。
- [ ] 持久结果继续只保存 `relevance/voice_type/sentiment/labels` 与既有身份字段；无 Schema/Migration、依赖或前端专用枚举复制。
- [ ] 受影响单元、Contract/API、Job/集成、质量和文档门禁取得当前 HEAD 新鲜证据。
...
| 编号 | 要求 | 来源 | 状态 | 证据 |
| --- | --- | --- | --- | --- |
| R1 | 模型输入严格限于五个既有业务字段 | user:优化真实用户判定 / AC1 | not_satisfied | 待 V4 Prompt、request payload 与输入 Hash 回归验证 |
| R2 | 实施 V4 Prompt、主体/意图拆分、证据输出和个人交易类别 | user:优化真实用户判定 / AC2 | not_satisfied | 待 Prompt/Taxonomy/解析实现与测试 |
| R3 | 增加本地语义一致性 Validator，不写关键词分类器 | user:优化真实用户判定 / AC3 | not_satisfied | 待 Semantic Validator 红绿测试 |
| R4 | 只有冲突/歧义内容进入 Judge，正常内容保持单次调用 | user:优化真实用户判定 / AC4 | not_satisfied | 待部分批次、请求种类与次数测试 |
| R5 | relevance、voice_type、targeted sentiment、多标签形成统一且兼容的 V4 链 | user:优化真实用户判定 / AC5 | not_satisfied | 待端到端 Service/Job/Contract 证据 |
| R6 | 暂不建立 Gold Set，参数调优与量化准确率评测随之延期 | user:优化真实用户判定 / AC6 | explicitly_deferred | 用户本轮明确“先不建立 Gold Set”；不把少量固定样例冒充 Gold Set |
| R7 | 完成后同步最新 main，通过门禁后合并到主分支 | user:优化真实用户判定 / AC7 | not_satisfied | 待提交、同步、CI、合并与主分支复验 |
...
- [ ] upstream_re_read：完成前重新读取当前用户 AC、相关正式文档、Prompt/Contract/实现和最终 diff。
- [ ] change_coverage：逐条核对 AC1-AC7 与实现、测试、文档和 Git 证据。
- [ ] reverse_audit：从公共 Result、数据库 Scheme、Worker、前端 Taxonomy/真实用户过滤和离线链反查兼容性。
- [ ] unresolved_cleared：清零 `not_satisfied`；延期项保留用户正式依据。
...
| --- | --- | --- | --- | --- |
| V1 | 待实现 HEAD | 目标 pytest | 待执行 | V4 协议、Validator、Judge 与 V3 兼容 |
| V2 | 待实现 HEAD | Contract/API/Integration | 待执行 | 公共边界和持久化链 |
| V3 | 待实现 HEAD | Ruff/mypy/质量脚本/CI | 待执行 | 静态质量、治理和完整回归 |
...
- CI：待执行。
- 合并：待最新 main 同步、门禁与 Review 通过。
- Change 归档：合并后由既有流程处理。
- 发布 / 部署：不在本次请求范围；既有数据库 Scheme 仍需显式发布。

## 备注

- 工作区已有 `.agents/` 受管文件修改，本 Change 不覆盖、不回滚、不提交这些用户改动。
