...
- Xiaohongshu Content image cache；Artifact TTL/容量回收与 Scheduler 吞吐；内部同源媒体读取；辅助补采预热；未补采懒缓存与媒体失败占位；声音广场详情媒体/评论状态展示；Figma 与必要文档/测试。
...
9. 媒体源暂不可用时内部路由返回固定静态 SVG 占位图并短缓存 60 秒；没有任何媒体事实时前端直接不渲染媒体区。
...
| 验证层 | 是否要求 | 范围 |
| --- | --- | --- |
| 单元 / Component | required | URL allowlist、图片类型/10MiB、TTL/容量、TTL 批次排空/max-batches、媒体路由、不可用占位、Collection prefetch、未采集评论状态；完整执行由 PR CI 证明 |
...
- [x] upstream_re_read：已重读本轮用户确认的 30 天/30GB/美观/Figma/合并要求，以及新增“未做辅助补采/评论采集仍要美观显示”的要求，并重新核对 Content/Artifact/Collection/API/Frontend/Figma Owner、Scheduler 清理入口与任务基线 main。
- [x] change_coverage：R1—R4、R6 已映射到真实实现、测试与 Figma 证据；R5 明确绑定 PR #477 全绿后的 post-merge finalization，没有把未来 merge 伪装成已完成。
- [x] reverse_audit：已反向核对 `Collection→prefetch→Artifact`、`Content media→内部 media route→Voice Plaza`、`未补采→lazy cache/neutral comments`、`expires_at→bounded hourly batches→30/24GiB capacity` 四条链路；业务 OpenAPI 与 TikHub 主链保持不变。
...
实现、测试用例、Migration、长期文档、Figma Normal Owner 与未补采/媒体回退状态规格已覆盖 Change 当前范围；Review 发现的每小时 100 条 TTL 吞吐缺口已改为有界批量排空并补测试。PR #477 的完整 CI 与独立 Review 作为 Ready 后强门禁继续执行，任何阻断 Finding 都会回到本 Change 修复。
