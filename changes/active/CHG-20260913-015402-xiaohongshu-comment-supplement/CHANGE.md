...
- **修改**：Canonical 媒体位置改为 `images_list` 数组顺序；用单元测试和 PostgreSQL/Fake Transport 纵切固定“媒体成功入库后继续请求评论”；Scope 的未预期异常增加不含 Raw/Secret 的安全诊断日志。
...
2. 扩展正式 Collection Runtime PostgreSQL 纵切，断言媒体位置唯一、评论请求被发出、评论成功入库。
3. 为 Scope 未预期异常补安全日志及日志泄密回归。
4. 扩大运行小红书、Collection Scope、评论/回复回归与静态检查。
5. 构建本地 Backend，使用现有辅助补采入口对目标内容有界重跑；验证 Provider Request、媒体、评论及现有评论 HTTP 输出。
6. 更新完成审计、Review、PR 检查；满足门禁后合并，读取主分支新鲜结果并由仓库自动化归档 Change。
