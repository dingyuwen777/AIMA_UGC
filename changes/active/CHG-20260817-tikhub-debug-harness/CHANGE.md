...
5. 内容唯一身份使用 `(platform, external_content_id)`；同次搜索和跨关键词/分页重复内容只执行一次后续详情/评论动作。评论按平台稳定 comment ID 去重；Provider 末页和目标达到后停止额外请求，已付费返回的整页数据不裁剪。
6. `tikhub_test/.env` 是本地运行配置，加载 TikHub URL/密钥；真实 `.env` 永不提交。仓库只提交 `.env.example`，Secret 不进入代码、日志、Raw、Canonical、manifest 或 Excel。
7. Excel 沿用已批准的 Real Provider Probe 格式：核心 Sheet 名为 `内容与评论`；一条内容形成纵向区块，公共内容列跨评论行合并，每条评论一行，保留 comment/root/parent ID，URL 可点击，ID 以文本保存，浅色表头、白色主体、无粗黑边框。
8. 自动化测试覆盖 `.env` 加载与 Secret 隐藏、状态去重、Raw/Canonical 落盘、XLSX ZIP/XML 完整性与布局、五个平台公开入口及无数据库依赖。
9. GitHub-hosted Runner 使用受控 Secret 对 `https://api.tikhub.io` 以关键词“爱玛”执行五个平台真实 Search → Detail → 一级评论 → 二级评论/回复验证；验证 Raw 与 Canonical 均可生成，且每个平台取得非空内容与评论样本。实时平台数据若某次检索没有可回复样本，必须扩大当前已批准搜索范围或明确报告证据，不能伪造通过。
...
- 不升级 Python、uv、前后端依赖；XLSX 优先使用 Python 标准库 OOXML，避免仅为调试导出增加依赖。
- 不把调试输出提交到 Git 仓库。
...
→ 修改范围：`backend/src/aima_ugc/adapters/providers/tikhub_test/{__init__,config,core,excel}.py`、`.env.example`、`output/.gitignore`  
→ 预期结果：配置安全加载；一次请求一次发送；Raw/Canonical/manifest/state/XLSX 可生成；内容和评论可跨运行去重。  
→ 验证方式：目标测试 + `uv run ruff check ...` + `uv run mypy backend/src`

### 3. Green：接入五个平台生产 Operation/Mapper

→ 修改范围：`tikhub_test/{xiaohongshu,douyin,weibo,bilibili,kuaishou}.py`  
→ 预期结果：五个平台入口仅准备参数、调用生产 Operation/分页/Mapper、保存输出，不复制 endpoint 或 Canonical 规则。  
...
当前仅确认开发基线：`main@e64a9e5956caf08fbbe14321cc0f45b603b3b919` 的适用 push workflows 查询结果均为 `completed/success`。本 Change 的 Red/Green、真实 Provider、PR 和合并后证据尚未执行，后续只记录本轮新鲜结果。

## Git 状态

- 分支：`agent/tikhub-test-debug`，已从 `main@e64a9e5956caf08fbbe14321cc0f45b603b3b919` 创建。
- Commit：本 Change 创建提交后开始记录。
- PR：尚未创建。
- 合并：尚未执行。
