...
收窄声音广场目录更新触发的投影刷新：无关字段更新不再枚举和重写关联 Content，真实影响投影的变更仍同步刷新。
...
| 编号 | 已确认事实 | 来源 | 支撑决策 |
| --- | --- | --- | --- |
| E1 | 0061 的目录 UPDATE Trigger 不区分字段，版本 touch 仍刷新全部关联投影 | Migration 0061、0068 上失败的集成测试 | 在 Trigger 中比较真实依赖字段 |
| E2 | 投影车型维度只读取品牌 role 与车型 merged_into_id | Migration 0061 投影刷新 SQL | 其他目录字段不必触发关联枚举 |
| E3 | 同一千条关联 Content 下无关更新重写一千条投影 | 隔离 PostgreSQL 18.4 基准 | 证明当前写放大的路径 |

推断与待确认：生产数据规模、P95 和锁等待需服务器实测；不阻塞已确认根因的修复。
...
必须保持不变：公共 API、目录与投影业务语义、事务内刷新和现有部署顺序。

# 约束与意图决策

目录由 vehicles 模块维护，投影由 content 读取；仅更改数据库 Trigger 判断。无公共接口或业务表结构变更。Migration 0069 可逆，异常仍按原事务回滚；部署遵循先迁移后应用的现有流程。
...
只在应用层跳过 no-op UPDATE 无法覆盖其他写入口；异步刷新会改变同事务一致性。因此选用数据库层 transition OLD/NEW TABLE 比较。
...
主要风险是遗漏真实依赖字段，已按投影 SQL 逐项反查并用 role/merge 集成测试覆盖。Migration 只替换函数，无回填；降级恢复旧函数后性能问题会重现，业务数据不丢失。公共 Contract 与配置保持兼容。

# 文档、依赖、部署与发布影响

已同步声音广场读模型运维文档。依赖、Runtime、Secret、公共消费方均无变化。发布需要先运行 0069 Migration；生产部署未获授权，也尚未执行。
