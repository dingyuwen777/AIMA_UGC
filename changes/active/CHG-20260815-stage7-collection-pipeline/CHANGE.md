...
affected_paths: [docs/blueprint/README.md, docs/blueprint/07-技术决策与实施门禁.md, docs/blueprint/08-采集策略与平台能力.md, docs/collection/]
contracts: []
data_changes: []
---
...
- [ ] 新增 `docs/blueprint/08-采集策略与平台能力.md`，作为 Stage 7 采集业务语义和平台能力的正式设计入口。
- [ ] 新增 `docs/collection/README.md`，说明通用抓取流程、决策顺序、成本控制、独立业务调试和文档事实层级。
- [ ] 为小红书、抖音、微博、B站、快手各建一篇采集逻辑文档，记录已批准 TikHub Operation 链、可配置业务参数、内部分页、平台差异、成本策略、代码/Fixture/测试状态和验收边界。
- [ ] Blueprint README 明确 Provider/采集开发必须先读 08，再读 `docs/collection/README.md` 和目标平台文档，然后才读代码/Fixture/测试。
- [ ] Blueprint 07 将版本推进并固化本轮跨模块决定，取消“其余四平台 Operation 尚未批准”的旧事实，同时保留真实脱敏 Fixture、实际字段兼容和 Scheduler misfire/catch-up 门禁。
- [ ] 文档明确：小红书当前已有 Operation/Mapper 机器实现；抖音、微博、B站、快手仍是 Stage 7 目标实现，不能把设计文档冒充代码事实。
- [ ] 文档明确：Stage 7 Provider/Collection/Capability/预算/Probe 可以开始按边界清楚的 PR 开发；Scheduler 启用仍等待 `misfire_policy/max_catch_up_runs` 批准；新平台完成验收仍需合法脱敏真实 Fixture 和 Real Probe。
- [ ] 不新增代码、Contract、Migration、依赖或锁文件；不泄露任何真实 Secret。
...
- 不把当前官方文档中无法稳定读取/尚未 Real Probe 的候选接口冒充已验收兼容性。
...
- 08 只冻结 Stage 7 跨模块采集语义、Capability、预算和 Operation Matrix。
- `docs/collection/` 面向开发/调试，按平台解释真实 API、当前代码状态和验证入口。
- Blueprint README/07 负责导航和权威门禁。
...
→ 验证方式：与 02/03/04/06/07、当前 XHS 代码和 TikHub 官方文档交叉复核。

[步骤 2：建立采集开发导航]
→ 修改范围：`docs/collection/README.md`、五个平台文档
→ 预期结果：开发某个平台时能快速确认实际 API、业务参数、内部参数、代码路径、Fixture/Probe 和成本停止条件。
→ 验证方式：本地 Markdown 链接门禁 + 与当前目录实际存在性核对。

[步骤 3：接入 Blueprint 导航和门禁]
→ 修改范围：`docs/blueprint/README.md`、`docs/blueprint/07-技术决策与实施门禁.md`
→ 预期结果：未来 Agent 自动被导航到 08 和目标平台文档；Stage 7 可开发范围与仍阻塞项明确。
→ 验证方式：文档入口/链接检查 + 术语/门禁冲突复核 + CI。

# 验证计划

本任务是 L3 设计/文档固化，不伪造 Red→Green。替代验证：

- 对照当前 main 的 XHS Operation/Mapper 和 Stage 6 事实；
- 对照 TikHub 当前官方文档核验五个平台主 endpoint、业务参数和分页状态；
- 检查所有新 Markdown 本地链接存在；
- 检查未把抖音/微博/B站/快手写成当前已实现；
- 检查未泄露 Secret、未硬编码真实 API Key；
- 以 PR GitHub Actions 的 `check_docs`、Secret、架构及现有回归作为新鲜集成证据；
- 合并后再次检查 main CI。

当前宿主无本地 Git 工作区/终端，因此不能提供本地 `git status` 或 `uv run ...` 输出；远端分支、diff、PR 和 GitHub Actions 作为本轮可执行证据。

# 兼容、Contract、Migration、部署和回滚

- 本 Change 只改变正式设计/导航，不改变当前运行行为。
- 公共 Contract/API：本 Change 无机器变化；Stage 7 实现时再按 08 建立版本化 Pydantic Contract/Capability。
- Migration/数据库：本 Change 无变化；`run_comments`/Provider 预算身份等是 Stage 7 目标设计，必须通过后续 L3 Migration 实施和 PostgreSQL 集成验证。
- 依赖/锁文件：无变化。
- 部署：无变化。
