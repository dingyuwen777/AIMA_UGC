...
1. 新增 `processing_import_batches`，作为 Excel File Import 的最小业务父事实，不复制 Content/Comment 业务字段。
2. `ProviderRequestV1/provider_requests` 从“只能属于 Collection Scope”一般化为“恰好属于 Collection Scope 或 Processing Import Batch 之一”。无父级和双父级均关闭失败。
3. Excel File Import **不制造** Collection Run/Scope/Candidate；使用真实 Input Artifact + Processing Import Batch + import-parent Provider Request/non-billable Attempt，并把 Input Artifact 绑定为 Attempt 的真实来源证据。
...
7. PostgreSQL 继续以 `(platform, external_content_id)` 和评论稳定身份作为跨批次、跨来源最终业务收敛边界，并保留 Version/Metric/来源历史。
...
- 数据库/Schema 失败明确失败；已经生成的调试文件不因 DB 失败被删除。
...
8. Migration/Contract/Unit/PG18/质量门禁：核心实现检查点已取得成功证据；最终合并仍必须以 PR 实际最新 head 的新鲜 CI 为准。

## 4. 来源链方案比较与最终选择

### 方案 A：Excel 制造 Collection Run/Scope

旧 Schema 改动少，但把本地文件读取伪装成外部 Collection Scope，污染采集 Run、页面统计和审计语义。**不采用。**

### 方案 B：一般化 Provider Request 父级

新增 `processing_import_batches`，使 `provider_requests` 恰好属于 `scope_id` 或 `import_batch_id` 之一，并继续复用 Provider Attempt、Artifact 和 Content 来源约束。既不伪造 Collection，也不复制 Attempt/Artifact 体系；既有 Collection 数据兼容。**已采用并实现。**

### 方案 C：独立 FileAttempt/FileSource 体系

会复制 Attempt/Artifact 概念，并迫使 Content Version/Metric/来源 FK、Contract 和 Repository 扩大为双来源模型。**不采用。**
...
Provider Request 已有 Attempt 后，`scope_id/import_batch_id/provider/operation` 继续受来源不可变 Trigger 保护。若已有 File Import Provider Request，Migration downgrade 明确拒绝，防止静默丢失 provenance。
...
`imports_test` 当前 API 是 `WRITE_TO_DATABASE=False`、`run_all(..., write_to_database=False)` 和 `ingest_database(run_dir=...)`。`run_all(write_to_database=True)` 顺序为 `convert → filter_keywords → deduplicate → database_ingestion → label_sentiment → export_labeled_excel`。数据库阶段失败时，之前已经生成的文件保留，后续 AI/最终 labeled Excel 不继续自动执行。
...
Green 后实现 Processing Batch、Provider Request 双父级、Excel 正式 File Import、两个调试入口数据库模式和 TikHub 正式来源桥接；后续只按 CI 证据修正 Ruff 格式/import-order 与 mypy 类型边界，没有降低门禁。专属 PG18 验收加入后曾因新测试文件 Ruff format 失败；业务 PG18 断言已成功，只格式化测试文件后重新取得全绿，未修改验收语义。
...
README、导航、测试说明和本 Change 收口后产生了新的文档提交，所以最终合并必须读取 PR **实际最新 head** 的完整适用 CI；不能用上述历史检查点替代最终证据。
...
稳定导航只有仓库 `dingyuwen777/AIMA_UGC`、分支 `feature/stage8a-unified-manual-ingestion`、PR #88、本 Active Change 与 Migration baseline `20260820_0019`。恢复时重新读取目标分支 `AGENTS.md` → Skill → 本 Change → PR 最新 head/CI；不在 Change 中维护动态分支 SHA。
