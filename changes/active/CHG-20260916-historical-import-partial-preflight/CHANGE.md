...
| --- | --- | --- | --- | --- |
| R1 | 正常文件 + 空文件时，空文件跳过且正常数据可导入，最终 succeeded | Issue #506 AC1 | satisfied | `historical_import_worker.py` 将空 Source 收敛为 `succeeded` warning；`test_historical_partial_preflight_postgres.py::test_good_and_empty_sources_import_good_data` 已提交，current-head PostgreSQL Integration 在 PR Ready 后执行 |
| R2 | 正常文件 + 坏文件时，正常数据可导入，最终 partial_failed 且坏文件可追溯 | Issue #506 AC2 | satisfied | `finalize_preflight()` 允许存在 failed Source 时进入 `ready`；`refresh_batch_and_campaign()` 纳入 Source failed；对应真实 DB/API/Worker 回归已提交，current-head PostgreSQL Integration 在 PR Ready 后执行 |
| R3 | 全空无写入成功；全坏 failed 且不可开始 | Issue #506 AC3 | satisfied | `finalize_preflight()` 明确全空 `succeeded` / 全坏 `failed`；两条真实 PostgreSQL/API/Worker 回归已提交，current-head PostgreSQL Integration 在 PR Ready 后执行 |
| R4 | 默认业务层使用用户语言，技术错误码只在技术详情追溯 | Issue #506 AC4 | satisfied | `DataImportDialog.vue` 显示“文件没有数据，已自动跳过。”和既有友好失败提示；技术详情保留 `relative_path · error_code` |
| R5 | 现有 Chunk partial failure、取消、重试、Filter 和两种写入策略保持不变 | Issue #506 AC5 | satisfied | 未改 Job type、Chunk retry、Filter Snapshot、Policy、Schema/Contract；PR current-head 全套 CI 作为最终回归门禁 |
