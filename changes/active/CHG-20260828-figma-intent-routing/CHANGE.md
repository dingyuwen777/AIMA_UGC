...
- [ ] PR #261 的 Change Completion Gate、Runtime Acceptance 和总 CI 全部通过后正常合并到 `main`，随后完成 Change 归档。
...
| --- | --- | --- | --- | --- |
| R1 | 把“全面检查 Figma 是否美观、好用、符合用户习惯和当前仓库实现”内化为短句可触发的流程 | user:内化全面检查提问 | satisfied | `SKILL.md` 3.1A：有目标实现时默认 `baseline-ready`，Design-only 时默认 `review-only`；后续既有章节继续承担视觉、可用性、Prototype、系统映射和 Design Context 门禁 |
...
| R3 | 内化时避免冗余，不复制两段长提示词和第二套 Coding 规范 | user:避免Skill冗余 | satisfied | 主 Skill 只增加意图路由；ref 05 只定义设计事实交接；`agents/openai.yaml` 已收敛为路由入口并明确以 `$figma` Skill/references 为唯一审查方法源 |
| R4 | 用户后续只需简单表达意图和给 Figma 链接即可使用 | user:后续简单说即可 | satisfied | README“最短使用方式”与 SKILL 3.1D 固化四类短句；目标仓库/分支/Git 只有无法从上下文确定且影响执行边界时再补 |
| R5 | Skill 继续跨项目通用，不局限 AIMA 或某种技术栈 | .agents/skills/figma/SKILL.md | satisfied | 新增路由使用 Route/Screen、Page/View、API/SDK/CMS/Local Store、Shared/Feature 等条件式通用模型；最终 diff 未新增 AIMA、具体页面、具体 Provider、具体技术栈或 Node 机器事实 |
| R6 | 交付必须走正常 PR/CI 路径，不直推 `main` 或绕过门禁 | user:推送主分支 | satisfied | Draft PR #261 已从 `docs/figma-intent-routing` 指向 `main`；当前 Change 在 PR 内进入 `ready_for_review`，最终 merge/归档仍等待 PR 门禁成功后执行 |
...
| Build / Package / Runtime | not_applicable | 不修改生产构建/包/runtime；YAML metadata 已按 YAML 语法解析，仓库 CI 负责当前 PR 的治理/解析门禁 |
| Docs / Governance / Other | required | 最终 diff 仅 4 个 Figma Skill 文件 + 当前 Change；README/SKILL/ref05/metadata 语义已交叉复核；Draft PR #261 已建立，待仓库三项门禁提供机器证据 |
...
- [x] reverse_audit：已执行“短用户意图 → 模式/流程”和“既有 review-only/review-and-fix/baseline-ready/Design-to-Code handoff → 自然语言入口”的双向审计；同时复核模式与写权限、Figma 与 Coding 责任边界，修正了 `review-only` 默认语义和“只读=模式”两处潜在歧义。
...
- [x] 创建 Draft PR #261 并将 Change 切到 `ready_for_review`。
- [ ] PR #261 三项 CI 门禁全绿后合并 `main`。
...
- 分支：`docs/figma-intent-routing`，基于 `main` SHA `ec13305e721b0010d65d1687c58f375751b8032e`。
...
- `agents/openai.yaml` 已用标准 YAML 解析器解析为 `interface.display_name / short_description / default_prompt` 结构，无 YAML 语法错误。
...
- 通用性审计：新增文本未写死 AIMA、Vue、Provider、具体 Route/Node/业务页面，条件式适配 Web/Mobile/Desktop/Design-only 等现有通用模型。
- Draft PR：#261 `文档：补强 Figma Skill 高频意图自动路由`，正常指向 `main`。

# 文档影响

- targeted：只同步 `figma` Skill 自身的 `SKILL.md`、README、ref 05 和 agent metadata。
- `AGENTS.md`、Blueprint、Figma Guide 没有新的产品/系统事实变化，不重复加入意图路由，避免形成第二套规则。

# 交付

- 开发分支：`docs/figma-intent-routing`
- PR：#261（Draft，待 Ready/CI）
- Merge：待 PR 门禁通过后执行
- Change 归档：待 main 合并后独立 PR 完成
