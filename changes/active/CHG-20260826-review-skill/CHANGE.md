...
| R8 | main 更新后必须基于最新主分支事实继续，不丢失新 README，并保持使用说明一致 | AGENTS.md | satisfied | 最新 `main` `eed6ebbb...` 已通过 merge commit `ee673e27...` 同步到 feature；compare `behind_by=0`；新增 `.agents/README.md`、`coding/README.md` 已 targeted 同步 Review，`docs/README.md`/Blueprint 06 经复核无需改 |

# Validation Matrix

| Layer | Required | Scope / Evidence |
| --- | --- | --- |
| 行为 / Unit / Component | required | Red HEAD `0ec7cf4c568b5ddf451d0c91a9b2de2ef8fd8d36`：Change Completion Gate `32873657217` 在 `Run Coding completion-gate tests` 失败；Green HEAD `987736b0b987d8eb0f864272b8ea9f9e40abef38`：run `32874908293` 的同一步 success；同步 main 后 run `32875393808` 再次证明 Skill tests 29/29 success |
...
| 用户 / Workflow Acceptance | required | `test_review_skill.py` 覆盖显式 Review、所有实现完成前 Review、Review 缺失/不可读边界、Review → Coding → re-review、Review README 及 `.agents`/Coding 人类使用导航；最终以本 Change 最新 HEAD 的 Change Gate 为准 |
...
| Build / Package / Runtime | not_applicable | 不修改 manifest、lock、镜像、构建或运行入口；同步 main 后的 feature HEAD 已触发现有 Runtime/Full-stack/CI 作为无回归补充，最终只采用最新 HEAD 结果 |
| Docs / Governance / Other | required | Coding 主 Skill 原文内容守恒；Review/routing/README 规则自动回归；最新 main README targeted 同步；Blueprint 06 和 Docs README targeted 复核无额外 diff；Ready Check 根因已从 workflow 日志定位并修正，不降低门禁；最终以最新 HEAD Change Gate/CI 为准 |

# Completion Audit

- [x] upstream_re_read：重新读取本轮“通用 Review、测试专家、Browser Mock、每次 Coding 路由、直接合入 main”要求、用户最新“README 必须位于 review 目录”的澄清，以及任务进行中更新后的 main 新 README 事实；重新读取当前 AGENTS、Coding 04/10/11、测试分层和 Docs 规则。
- [x] change_coverage：独立重建的要求均映射到 R1—R8；README 路径已按最新澄清修正；main 新增人类使用说明已纳入真实影响，不把 PR 描述或当前 Change 自身当需求全集。
...
- [x] unresolved_cleared：R1—R4/R6—R8 已 satisfied；R5 按 AGENTS 强制集成顺序正式延期到 Ready + CI 后同任务执行，没有功能性 `not_satisfied`；所有 Validation `not_applicable` 均有事实依据。
...
R1—R8 已覆盖 1—10；第 10 项按仓库规定只能在 Ready 后完成集成，因此保留为正式 integration-stage deferred，不冒充已经合并。
...
- Red/Green 已实际由 GitHub Actions 建立；同步 main 后 Skill tests 仍 29/29 success；Ready Check 的一次失败已由 job log 精确定位为 R7 Source 拼接两个路径，修正为合法单一来源 `AGENTS.md`，没有降低门禁；
- PR patch 人工复核证明 Coding 主 Skill 原有正文没有被重写，只在末尾追加 Review 集成；路由 reference 仅修改 Review/Audit 段（另补 EOF newline）；
- main 在分支开发期间从 `9412286...` 更新到 `eed6ebb...`，新增 `.agents/README.md`、`coding/README.md`、`docs/README.md`；最新 main 已合并入 feature 且 `behind_by=0`；
- Docs Impact = `targeted`：`.agents/README.md` 与 `coding/README.md` 会因 Review Skill 新增而事实过期，已最小同步；`docs/README.md` 仍正确要求 Code Review 从 Coding 统一入口进入；Blueprint 06 仍正确描述上位流程，具体 Review 路由复制过去会形成第二套规则，所以后两者不制造 diff。
...
- 无关改动：Docs targeted 只修改新 main 引入且确实受影响的 `.agents/README.md`、`coding/README.md`；`docs/README.md` 与 Blueprint 06 经复核保持无 diff；
...
- branch: `feature/review-skill`
- PR: `#240`（Draft，待最新 HEAD Ready Check 与全量 CI）
- base sync: 最新 `main` `eed6ebbb8348827998bba56f46c6e3cf83eacd13` 已通过双父 merge commit `ee673e27be40fdf5521b24bd2dfb10eca1f6c5d0` 合入 feature；compare 显示 merge base 为该 main SHA、feature `behind_by=0`
- Red evidence: `0ec7cf4c568b5ddf451d0c91a9b2de2ef8fd8d36` / Change Completion Gate `32873657217`
- Green skill-test evidence: `987736b0b987d8eb0f864272b8ea9f9e40abef38` / Change Completion Gate `32874908293`；同步 main 后 run `32875393808` 仍为 29/29 success
- Ready Check previous root cause: run `32875393808` 仅因 R7 Source 拼接两个路径失败；workflow job log 明确报 `Requirement Source 仓库文件不存在`，已按机器规则修正为单一真实来源 `AGENTS.md`
- main merge: 待本 Change 最新 HEAD Ready + 全量 CI 绿后执行
