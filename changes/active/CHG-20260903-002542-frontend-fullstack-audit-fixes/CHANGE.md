...
- [ ] 不修改 HTTP Contract、OpenAPI/generated client、数据库 Schema/Migration、依赖、Runtime、采集/导入/AI/导出业务语义。
...
- 不升级 Vue、TypeScript、Python、PostgreSQL、npm/uv 依赖或 Runtime。
...
| R6 | 不修改 Contract/Schema/依赖/业务语义，并完成系统黑盒与真实全栈验证后合并 main | https://github.com/dingyuwen777/AIMA_UGC/issues/310 | not_satisfied | 待 diff、CI、Review、PR/main fresh 证据 |
...
| Build / Package / Runtime | required | frontend lint、typecheck、Vitest、Vite build、Playwright；仓库 Runtime Acceptance |
...
- [ ] change_coverage：逐项从 Issue #310 重建 R1-R6，确认没有遗漏已审计问题。
...
- [ ] 完成最小实现，不修改公共 Contract/Schema/依赖。
...
- 首次分支 CI 必须观察到这些测试因当前旧实现失败；若出现编译/环境失败，先修测试，不把环境错误当 RED 证据。

## GREEN 计划

- Targeted Vitest：新回归 + identity/import/voice/admin 相关既有测试。
- Frontend 全量：lint、typecheck、Vitest、build。
...
- 待 RED / GREEN / PR / main fresh 执行后填写。
