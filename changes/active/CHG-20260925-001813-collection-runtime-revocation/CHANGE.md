...
数据导入撤销后，采集运行中心列表接口返回 409，页面无法加载。统一运行投影透传 Campaign 的 `revoked` 状态，但公共运行状态不接受该值。计划只修正读取投影及必要的阶段标签，保持导入和撤销写入路径不变；并以本地运行日志核查全流程性能与资源自适应边界。上游需求：#601。
...
| 编号 | 要求 | 来源 | 状态 | 证据 |
| --- | --- | --- | --- | --- |
| R1 | 撤销中、撤销后列表与筛选可用，阶段准确且无错误补采入口 | #601 / AC1 | not_satisfied | 待回归测试 |
| R2 | 修复不降低数据处理效率 | #601 / AC2 | not_satisfied | 待代码边界和测试复核 |
| R3 | 量化运行耗时、堵点与跨服务器资源调整范围 | #601 / AC3 | not_satisfied | 待日志分析 |
| R4 | Review、CI 通过后合并 main | #601 / AC4 | not_satisfied | 待交付 |
