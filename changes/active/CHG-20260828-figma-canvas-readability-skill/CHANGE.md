...
- [ ] PR #267 在当前 Ready HEAD 永久门禁全绿后转 Ready 并正常合并 `main`，再验证 merge 后 `main`；这是 Ready 后的交付动作，不在门禁前提前执行。
...
| --- | --- | --- | --- | --- |
| R1 | Figma 页面内画板、注释说明等必须保持合理间距，保证画布美观与可读性，并把该要求写入 Skill | user:2026-08-28-figma-canvas-spacing | satisfied | 新增 `13_Figma设计画布与可读性规则.md`，覆盖页面、注释、画板组、分区和 Canvas-level Review；`02_跨项目研发任务路由.md` 已把 Figma 任务硬路由到该 reference；CI #3370 新增回归测试通过 |
| R2 | Figma 页面/公共组件优先遵循当前项目已有 Design System/Spacing，而不是另造第二套规则 | docs/guides/01_Figma与前端设计开发工作流.md | satisfied | 新 reference 明确“项目 Design System/Token/Grid/Layout → 当前稳定模式 → fallback”的优先级；AIMA Guide 已有 Spacing Design Token 与视觉间距核对，不复制第二份固定数值表；docs check 通过 |
| R3 | Detailed reference 只有命中触发条件时才加载，主 Skill 保持路由入口而不堆叠全部细则 | .agents/skills/coding/SKILL.md | satisfied | 主 Skill 强制每个任务先读取 `02_跨项目研发任务路由.md`；本次只在该路由增加 Figma 命中条件，并把详细规则放入独立 `13_` reference；Coding governance 42/42 通过 |
| R4 | 完成后正常推送并合并主分支，不绕过仓库门禁 | user:2026-08-28-push-main | explicitly_deferred | PR #267 已创建；按 `AGENTS.md` 的 Ready→Completion Gate/CI/Review→merge→main 新鲜验证顺序，在当前 Change Ready 后继续执行 |

# Validation Matrix

| Layer | Required | Actual Evidence |
| --- | --- | --- |
| 行为 / Unit / Component | required | PR HEAD `60a5a237fba08275b885910cf29bb0b1e78a1e16`，CI #3370 / Docs and Governance：`python3 -m unittest discover .agents/skills/coding/tests -v` 共 42/42 passed；新增 `test_figma_canvas_readability_guidance_is_routed_and_actionable` passed |
| 接口 / Contract | not_applicable | diff 仅 Coding references/test/Change，不修改 public API/ABI/HTTP/Schema/format/generated boundary |
...
| Build / Package / Runtime | not_applicable | 不修改可执行包、Runtime、Manifest、锁文件或依赖；Runtime Acceptance #491 按 unchanged-runtime fast-path success |
| Docs / Governance / Other | required | CI #3370：Secret scan、`check_docs.py`、Coding governance 42/42、CI Gate success；首轮 Change Gate #1216 的 completion-gate tests success，readiness 仅因当时 Change=`in_progress` 按设计阻塞 |
...
6. Draft PR #267 首轮 CI：CI #3370 success，Coding governance 42/42 passed；Runtime #491 unchanged-runtime fast-path success；Change Gate #1216 的规则测试成功、changed-PR readiness 因 Change 仍为 `in_progress` 按设计失败。
7. Ready 前重新读取 `AGENTS.md`、`SKILL.md`、AIMA Figma Guide 和 Review Skill，重新审查 branch diff；当前仅 4 个治理文件变化，无产品代码、Contract、依赖或 Figma 文件变化。
...
- branch: `docs/figma-canvas-readability-skill`
- Ready 前实现 HEAD: `60a5a237fba08275b885910cf29bb0b1e78a1e16`
- PR: #267 `文档：增加 Figma 画布可读性与间距规则`（Draft）
- merge: pending；仅在当前 Ready HEAD 的适用永久门禁全部成功后执行

# Completion Audit

- [x] upstream_re_read：Ready 前重新读取当前分支 `AGENTS.md`、Coding `SKILL.md`、AIMA Figma Guide、Review Skill；独立确认用户要求是“Figma 页面内图/注释说明保持合理间距和整体美观可读，并写入 Skill”，现有 Guide 已有项目级 Spacing/视觉核对但缺少 Canvas hygiene 细则。
- [x] change_coverage：R1–R4 逐项重建。R1 的 Canvas/注释/画板可读性、R2 的项目 Design System 优先、R3 的按触发条件加载详细 reference 均已由当前 diff 与治理测试覆盖；R4 仅为遵守正式 Ready→merge→main 验证顺序显式延期。
...
- [x] unresolved_cleared：PR #267 当前无 inline review thread/submitted review；首轮 CI 没有规则实现失败，唯一失败是 `in_progress` Change 的预期 readiness 阻塞；A1/A2 未发现 BLOCKER/HIGH/MEDIUM Finding，`not_satisfied` 已清零。
...
- branch 相对 base 只修改 4 个治理文件：`02_` 路由、`13_` 新 reference、Coding governance test、当前 Change；没有生产代码、Contract、Migration、generated client、依赖或 Figma 资产变化。
- 新规则采用“项目 Design System 优先 + 通用 fallback”的条件结构，避免把 AIMA 当前数值或个人审美硬编码为跨项目强规则。
- 规则明确了最小范围：修复当前修改直接造成的邻接问题，但禁止借 Canvas 整理扩大到无关全文件重排或擅自删除历史资产。
- CI #3370 真实运行全部 Coding governance tests 42/42 passed，其中新增 Figma 路由/可执行性测试通过；Secret scan、docs check、CI Gate 均通过。Runtime #491 证明本次未触发 Runtime 风险路径。
- 没有必要新增产品 E2E、PostgreSQL、Contract 或真实 Figma API Probe；这些层对本次纯治理规则没有独立证明价值。
- 容器环境此前无法解析 GitHub 域名，因此没有把本地未运行验证写成通过；当前结论以 GitHub Runner 新鲜证据为准。
