...
| R5 | 永久回归防止 env/Compose/launcher 再次漂移 | #364 / AC5 | satisfied | `tests/unit/test_env_compose_config_contract.py` 直接锁定共享 env、legacy fallback、host/runtime target、文档/Tooling入口，并要求已失效 `AIMA_ANALYSIS_RUN_SHARD_SIZE` 重新被识别为 unknown。 |
...
| 验证层 | 是否要求 | 范围 / 证据 |
| --- | --- | --- |
| 行为 / 单元 / 组件 | required | `test_env_compose_config_contract.py`：launcher env 解析、host/runtime 路径选择、legacy fallback、unknown-key 分类。 |
...
- [x] reverse_audit：从 env 示例反查 Compose、`Settings`、`local_runtime.py`、Analysis Shard 计算、Windows Guide 与 Tooling；发现并修复 Windows Guide/Tooling 的旧 `env.production` 假设，并删除无消费者的 `AIMA_ANALYSIS_RUN_SHARD_SIZE`。
...
- PR #365 current diff：10 个文件，范围均属于本 Change；`.github/workflows/tooling.yml` patch 仅把 Windows 本地 Compose 从 `env.production` 切换为 `env.local`，没有整文件语义漂移。
...
- 新增配置契约测试的真实 test result。
- Developer Tooling：Linux launcher/bootstrap 与 Windows Compose CLI current-head 结果。
- Runtime Acceptance：canonical Compose startup/readiness/persistence current-head 结果。
- 项目 Requirement/Completion/docs/CI gate current-head 结果。
