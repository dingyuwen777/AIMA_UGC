...
- [ ] 仓库 Figma 工作流 Guide 同步上述长期边界并重新读取验证。
...
| R5 | 仓库正式 Figma 开发工作流同步上述边界 | user:可以更新文档 | not_satisfied | 本 Change 后续更新 `docs/guides/01_Figma与前端设计开发工作流.md` |
...
| Docs / Governance / Other | required | 更新 Figma 工作流 Guide；重新读取目标文档与 Change，检查与当前 routes/Contract/Guide 一致 |
...
- [ ] unresolved_cleared：待 Guide 更新并重新读取后清除 R5 `not_satisfied`。
...
- `docs/guides/01_Figma与前端设计开发工作流.md`：补充目标 IA 与真实 Route 分层、动态数据事实源、采集策略正式 Figma 基线和 Codex 实施规则。

# 交付

- Commit：GitHub Contents API 会在独立文档分支产生中文文档提交；不提交生产业务代码。
- PR：本轮未授权创建。
- 发布：不适用。
