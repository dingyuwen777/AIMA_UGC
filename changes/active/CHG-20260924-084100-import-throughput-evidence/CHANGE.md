...
对 Excel、统一历史导入与全历史 Canonical Replay 共用的 Content 写入链路做证据驱动的第二阶段优化：批量收敛稳定作者/备用身份和新旧 Content，把声音广场逐行 Trigger 改为语句级集合刷新，同时保留提交后立即可见、可取消/接管/撤回与持久证据语义。
...
- 作者/账号与 Content 的业务语义、freshness、人工锁、唯一约束、来源贡献和 Replay 撤回必须保持。
- 声音广场按用户最新决定保持强一致：入库事务提交后的下一次查询即可见，不引入后台投影等待窗口。
- 全历史 Canonical 首次重筛仍完整预检；已有验证证明可复用，文件或规则变化时失效，不以跳过完整性校验换取速度。
- 性能验证必须有硬磁盘预算、专用数据库和可审计清理；不运行生产 Migration、Release、Deploy 或生产数据操作。
- 不降低 PostgreSQL durability，不删除持久 Raw/Input/Canonical 证据，不升级依赖或引入新基础设施。
...
Docs Impact 为 targeted：同步统一入库实现、4000 万容量/排障运行手册，以及 Migration head/部署注意事项实际受影响的现有文档。不创建新的平行架构说明。没有依赖、Runtime 或公共 Contract 变更；服务器发布必须先由正式 migrate 进程升级到 `20260924_0061`，再启动新 API/Worker。本任务不执行 Release 或 Deploy。
