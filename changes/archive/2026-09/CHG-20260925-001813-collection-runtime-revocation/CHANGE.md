...
- 成功标准以 #601 AC1—AC6 为准：运行列表与详情可用，失效来源被跳过且其他来源继续，同六文件对照与状态计数可复核，Worker/Job 窗口随有效资源变化。真实服务器的绝对最优吞吐不在本机验收范围内。

# 约束与意图决策

- 用户明确要求失效来源跳过，而不是使整个历史重筛停止；撤销普通导入不会自动触发新的全量重筛。撤回档位无收益时先回退，稳定一段时间后可重新探测同一高档。
- 用户要求不同宿主自动调整 Worker 与 Job 窗口，并保留宿主余量；启动脚本只能使用 Docker Engine 可见资源，不能突破 Docker Desktop 虚拟机限制。
- 当前公共状态、Contribution Ledger、Campaign Chunk 冻结语义和 PostgreSQL Owner 边界均维持；运行中只调整后续调度与批次，不重切既有 Chunk。
...
- 让撤销后重筛整体失败会继续阻断其他有效来源，不满足用户确定的跳过语义；逐批复核加 checkpoint 能使已冻结请求继续完成。
- 固定两个 Worker 或直接按宿主总核数启动进程都不能反映容器配额及数据库竞争；按 Worker 有效预算和队列逐步扩缩，保留启动脚本划出的宿主余量。
- 固定提高撤回批量到 1000 在本机对照中增加了墙钟；保留相同 500 起步与步长，由实时资源、事务耗时和相邻档吞吐决定是否继续探索。高档无收益时回退，稳定批次后重新探测，以适应运行中负载变化。
...
| R6 | 重筛撤回不逐条更新已由后续来源接管的账本，并对升档做实测取舍 | #601 / AC2 | satisfied | 101 条后续普通导入接管的 PostgreSQL 集成测试，撤回期间 SQL 总数小于 100；同六文件 Docker 撤回旧路径 122.7 秒、优化运行 105.3 / 97.7 秒；放宽本机探索上限后的反例为 120.8 秒，已拒绝该档位策略；原有定时复测在本次执行中保留 |

# 计划改动

| 步骤 | 修改范围 | 可观察结果与验证 |
| --- | --- | --- |
| 运行投影 | Collection 查询、前端阶段标签、HTTP/前端测试 | 撤销中/已撤销仍可分页，真实阶段可见，列表 200 |
| 重筛来源与事务 | Canonical Replay 仓储/Worker、PostgreSQL 集成测试 | 失效来源被跳过，检查点推进，其他来源继续且无撤销竞态 |
| 资源与吞吐 | Compose 启动、容量计算、Worker 进程池、撤回账本、容量/Job 测试 | 不同资源预算得到不同上界，运行中升降档并保留数据统计 |
| 文档与运行验证 | 运行/导入文档、隔离 Docker 六文件 A/B | 各阶段耗时、瓶颈、风险和回滚边界可复核 |
